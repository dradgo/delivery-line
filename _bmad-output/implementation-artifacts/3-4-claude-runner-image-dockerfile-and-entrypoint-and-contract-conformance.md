...
- [x] **Task 7: docker-compose build target** (AC: 5)
  - [x] Add a `claude-runner` service to `docker-compose.yml` alongside `codex-runner`: `build: { context: ., dockerfile: runners/claude/Dockerfile }`, `image: deliveryline/claude-runner:latest`. Build-only image target, profile-gated (`profiles: ["runners"]`) so Spring Boot's compose-`up --wait` does not try to run the one-shot runners; `docker compose --profile runners build` rebuilds **both** runner images, and `docker compose build claude-runner` builds it by name (AC5). Keep the single-file compose convention.
...
- [x] [Review][Patch] AC5 compose command semantics conflict with runner profiles [docker-compose.yml:28, docker-compose.yml:38] — decision resolved 2026-05-31: removed runner profiles so plain `docker compose build` includes both runner images. **Reverted 2026-05-31 (CI fix):** removing the profiles regressed the `doctor-smoke` tier — Spring Boot 4's docker-compose autoconfig runs `docker compose up --wait` against this file, and the unprofiled one-shot runners exit non-zero, failing the wait and aborting app startup (`container deliveryline-claude-runner-1 exited (2)`). Profiles are the only per-service exclusion the Spring Boot compose lifecycle honors, so `profiles: ["runners"]` is restored on BOTH runner services. AC5 reconciled within the single-file convention: build by name (`docker compose build claude-runner`) or rebuild both with `docker compose --profile runners build` — naming/activating a profiled service still builds it (verified).
