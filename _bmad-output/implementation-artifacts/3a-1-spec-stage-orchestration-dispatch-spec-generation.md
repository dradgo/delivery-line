...
- [ ] [Review][Patch] **NOT APPLIED (batch-skipped — requires judgment / potentially intentional)** `onSpecStageSucceeded` swallows ALL `ILLEGAL_TRANSITION` — masking a genuinely anomalous source state (run concurrently taken-over/failed/reconciled) as if it were a benign idempotent replay. The story DESIGNS this as "idempotent swallow ILLEGAL_TRANSITION" (Javadoc :204-206), so narrowing it changes documented semantics. Suggested fix if accepted: narrow the swallow to the replay case (current state already at/after `WaitingForSpecApproval`); rethrow/elevate on a genuine anomaly. [WorkflowOrchestrationService.java:208-245] (sources: blind, MEDIUM)
- [ ] [Review][Patch] **NOT APPLIED (batch-skipped — deliberately documented behavior)** `submit` returns `WorkflowState.INBOX` after the run was auto-advanced to `INVESTIGATING` in the same transaction. The dev comment (`WorkflowCommandService.java:206-207`) explicitly documents returning the "just-created Inbox state" by design. Suggested fix if accepted: re-read and return the committed current state. [WorkflowCommandService.java:208-212] (sources: edge, LOW)
