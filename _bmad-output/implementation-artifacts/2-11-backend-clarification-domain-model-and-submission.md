...
| **T12** | `answerText` and `questionText` are reviewer/runner-supplied free-form text. Logging or surfacing them in error responses risks leaking sensitive product content. | **Forbidden in log output and Problem Details details map.** Use `.length()` if presence-signaling is needed. Mirror story 2.10's `reasonText` redaction. |
