...
| 2026-05-29 | `in-progress → done` at story 3-2a close-out. Story 3-2a discharged the open review action items (HIGH stage-starvation, HIGH dangling-sweep safety, AC9/T13 retry event-id anchor via a new `RetryRecoveryResult.runnerDispatchedEventPublicId` field, detectHeartbeat atomicity + recover re-seed, event double-emission gate, `processHeartbeatStale` null) + authored the full AC10 test surface (~22 tests green locally) + verified `OpenApiSnapshotContractTest` on a real springdoc boot + ran the `docker-runner-it` tier green on WSL2 Ubuntu. 2 MED items (same-tick stale boundary shared-`now`; non-atomic workspace delete give-up policy) were NOT in 3-2a's ACs and are moved to `deferred-work.md` as accepted tech-debt. 3-2a also fixed a critical regression where 3-2's `RunnerProperties` validated fields were missing from the test `application.yml`, breaking the whole `@SpringBootTest` tier. |
...
- [x] [Review][Patch][RESOLVED in 3.2a — anchor surfaced via a NEW `RetryRecoveryResult.runnerDispatchedEventPublicId` field (not `recoveryRetriedEventPublicId`, to preserve the recovery.retried contract); `RecoveryServiceDockerRetryContractTest` covers timed_out + orphaned] MED — Build the AC9/T13 retry event-id anchor + contract test (Decision D3) [RecoveryService / RunnerBroker.dispatch / RetryRecoveryResult] [auditor F7] — **NOT batch-applied (cross-cutting; must land with its test).** Thread the new `runner.dispatched` event public id into `RetryRecoveryResult.recoveryRetriedEventPublicId` under `runners.docker`; add `RecoveryServiceDockerRetryContractTest` asserting (a) fresh `rex_*` id, (b) original row status unchanged, (c) the event-id anchor is the `runner.dispatched` id. Build deliberately in the next session.

- [x] [Review][Patch][RESOLVED in 3.2a — added stage-scoped `findStaleByStatusInAndStageAndLastActivityAtBefore` (port+adapter+repository) + per-stage scan; `RunnerBrokerStaleDetectionUnitTest` starvation case] HIGH — Stale-scan stage starvation: query is batch-limited but NOT stage-scoped; per-stage in-memory `continue` discards rows [RunnerBroker.scanForStaleExecutions; RunnerExecutionPersistenceAdapter.findStaleByStatusInAndLastActivityAtBefore] [edge#2 / blind#1 / auditor F8] — **NOT batch-applied (cross-layer port signature change).** Under backlog the oldest rows of one stage consume the `batchSize` LIMIT and starve the other stage every tick. Add a `stage` predicate to the port query (touches port + adapter + repository) and land with a starvation unit test.
- [x] [Review][Patch][RESOLVED in 3.2a — added `danglingContainerMinAgeSeconds` min-age guard, normalized `getState()` status, two-pass stop→force-rm; `RunnerWorkspaceCleanupJobDanglingUnitTest` + `DockerRunnerDanglingContainerCleanupIT`] HIGH — Dangling-container sweep can destroy a live just-created container & leaks running ones [RunnerWorkspaceCleanupJob.sweepDanglingContainers] [blind#9 / edge#5 / auditor F4] — **NOT batch-applied (needs a config threshold + design).** (a) no `createdAt` min-age guard → a container in the dispatch→row-insert window (row absent) is stopped+removed; (b) status from `getStatus()` ("Up 3 minutes") isn't matched by the `running|paused|restarting` guard → `stop` skipped, then `rm force=false` 409s and leaks; (c) AC6 "two-pass stop-then-rm after exit" not implemented. Add a min-age config + guard, normalize status, implement two-pass/force-aware removal.
- [x] [Review][Patch][Deferred→deferred-work at 3.2a close-out — NOT in 3.2a ACs; largely mitigated already (`processHeartbeatStale` skips rows past the orphan cutoff, so a row past 2× never double-emits); residual is only the exact-`2×timeout` microsecond boundary. Shared-`now`/operator-alignment refactor deferred] MED — Stale two-pass boundary + same-tick double emit [RunnerBroker.scanForStaleExecutions / processHeartbeatStale] [edge#1 / edge#1b] — **NOT batch-applied (timing refactor).** Phase 1 & Phase 2 use independent `now()` reads and `isBefore` vs strict `<`; a row can emit `runner.heartbeatStale` AND `runner.orphaned` in one tick, or get a warn instead of an orphan flip at the exact `2×timeout` boundary. Thread one shared `now`; align comparison operators.
- [x] [Review][Patch][RESOLVED in 3.2a — append now gated on the locked transition (catch `ILLEGAL_TRANSITION` race) in `processStaleOrphan`/`processOrphan`/`handleSuccess`; `RunnerBrokerStaleDetectionUnitTest.orphanPathInvokedTwiceEmitsRunnerOrphanedExactlyOnce`] MED — `RUNNER_ORPHANED`/`RUNNER_COMPLETED` event double-emission [RunnerBroker.processStaleOrphan / processOrphan / onResult] [blind#7 / edge#12 / auditor F6] — **NOT batch-applied (needs transition-result semantics).** Event append is not gated on the row actually transitioning; recovery replay re-entering `onResult`, or concurrent scan vs startup-recovery, can append duplicate lifecycle events. Gate the append on the actual status transition.
- [x] [Review][Patch][RESOLVED in 3.2a — remaining (a) atomic `merge` high-water-mark + (c) `recoverHandle` observation re-seed now applied; `DockerRunnerAdapterUnitTest` adds empty-log, ms-precision, concurrent-non-regression, recover-reseed cases] MED — `detectHeartbeat` correctness cluster [DockerRunnerAdapter.detectHeartbeat] [blind#3 / blind#4 / blind#12 / edge#3a / edge#3b / edge#11] — **APPLIED:** (b) empty `runner.stdout` no longer registers as growth on first poll (baseline `0`, not `-1`); (d) Trap-T3 ms-precision now actually implemented via `truncatedTo(MILLIS)` in `LocalRunnerWorkspaceStore` (comment was false). **NOT applied (needs concurrency restructure):** (a) ConcurrentHashMap get→put atomicity (`compute`/`merge`); (c) re-seed `rexIdToLastLogObservation` on `recoverHandle` so a restart doesn't re-emit `HeartbeatTouched(stale startedAt)`.
...
- [x] [Review][Patch][RESOLVED in 3.2a — explicit null-`lastActivityAt` skip + WARN in `processHeartbeatStale`; `RunnerBrokerStaleDetectionUnitTest.heartbeatStaleSkipsRowWithNullLastActivity...`] LOW — `processHeartbeatStale` silently swallows a null `lastActivityAt` [RunnerBroker.processHeartbeatStale] [blind#2] — **NOT batch-applied (low value: the SQL `< cutoff` predicate won't select null-activity rows anyway).** Handle null explicitly if a null-activity stale row ever materializes.
