...
- [x] **Task 1 — Backend: `preview-rerun-from-step` read endpoint (AC5, AC12)**
  ...
  - [x] Add response DTO `PreviewRerunFromStepResponse(String workflowRunId, String targetStep, List<String> supersededArtifactIds, List<String> invalidatedApprovalIds)` — both lists REQUIRED + never-null (mirror `RerunFromStepResponse`'s REQUIRED never-null pair so the generated TS client can rely on them). Add a `from(...)` factory.
  - [x] Reuse existing error codes only — NO new `DomainErrorCode`: `INVALID_RERUN_TARGET_STEP` (400), `ILLEGAL_TRANSITION` (409) for wrong source state (or return an empty preview for non-rerunnable states — see Open Question OQ-1), `RUN_NOT_FOUND` (404), `INVALID_ID_PREFIX` (400) via `PublicIdPrefixes.require` in the service.
  - [x] Regenerate the OpenAPI snapshot: run `OpenApiSnapshotContractTest` with `-Dopenapi.snapshot.write=true` (writes then `fail()`s — expected), review + commit the `openapi.json` diff, re-run to confirm byte-identical; add an operationId assertion for `previewRerunFromStep`. Then `cd deliveryline-frontend; npm run generate-api`.
  - [x] Tests: `@WebMvcTest`/contract test for the endpoint (happy path returns both id lists; `targetStep=bogus` → 400 `INVALID_RERUN_TARGET_STEP`; wrong source state → per OQ-1; unknown run → 404); a `RecoveryService` unit/integration test proving `previewRerunFromStep` performs **zero writes** (no `recovery_actions`/`workflow_events` rows, no approval invalidation).
- [x] **Task 2 — FE: extend the action vocabulary + safety model (AC1, AC2)**
  - [x] In `deliveryline-frontend/src/features/workflows/approvalDecisionView.ts`: add `resume_workflow`, `reconcile_conflict`, `rerun_from_step`, `pause_workflow`, `classify_failure` to BOTH the `DecisionAction` union AND the `KNOWN_ACTIONS` set — otherwise `normalizeActions` coerces them to `unknown` and drops them.
  - [x] Add a `RecoverySafetyLevel = 'safe' | 'caution' | 'risky'` type + `resolveRecoverySafety(token, diagnostics)` helper that joins each allowed-action token to a `FailureDiagnosticsResponse.recommendedRecoveryActions[].safetyLevel` via a **token→actionType map** — the diagnostics vocabulary is SHORT-FORM and differs from the wire tokens: `{ retry→'retry', pause_workflow→'pause', reconcile_conflict→'reconcile', classify_failure→'classify_failure' }`. `rerun_from_step` and `resume_workflow` are ABSENT from diagnostics (rerun isn't recommended; resume only applies to `Paused` runs, which produce no diagnostics), so give them a **static fallback**: `rerun_from_step → 'risky'` (it supersedes artifacts + invalidates approvals — matches AC5's `danger` styling), `resume_workflow → 'safe'` (the expected forward action from `Paused`). Any token with no diagnostics match and no fallback → `'caution'`.
  - [x] Add `resolveRecoveryPrimaryAction(actions, safetyByToken)`: returns the single `safe`-rated action to style primary, with a deterministic tie-break priority order (e.g. `retry` > `resume_workflow` > `reconcile_conflict` > others) so multiple `safe` actions never both go primary. Returns `null` → no primary (bar shows subordinate-only / view-only).
  - [x] Extend `CONSEQUENCE_HINTS.recovery_operator` with short inline hints for `resume_workflow`, `rerun_from_step`, `pause_workflow` (reconcile/classify hints live in their dialogs, 4.23/4.24). Add unit tests in `approvalDecisionView.test.ts` for the new coercion, safety mapping (including the short-form join + fallbacks), and primary resolution.
- [x] **Task 3 — FE: new mutation + query hooks (AC3, AC5, AC6, AC8)**
  - [x] `useResumeWorkflow(workflowRunId)` → `POST /api/v1/workflows/{workflowRunId}/resume`, body `ResumeWorkflowRequest { role: 'workflow_owner', reasonText? }`. Build on the `useWorkflowMutation` factory (inherits UUIDv7 `Idempotency-Key` + `detail(id)`/`lists()` invalidation). Use the `RECOVERY_OPERATOR_ROLE` constant for `role` — do NOT copy `useRetryWorkflow`'s `actorIdentity`/`actorType` body pattern (resume/rerun/pause use a header-derived actor + a `role` body field).
  - [x] `useRerunFromStep(workflowRunId)` → `POST /api/v1/workflows/{workflowRunId}/rerun-from-step`, body `RerunFromStepRequest { role, targetStep, reasonText }`. Vars `{ targetStep: 'investigating'|'executing', reasonText: string }`. Response `RerunFromStepResponse`.
  - [x] `usePauseWorkflow(workflowRunId)` → `POST /api/v1/workflows/{workflowRunId}/pause`, body `PauseWorkflowRequest { role, reasonText }`.
  - [x] `usePreviewRerunFromStep(workflowRunId, targetStep, { enabled })` → TanStack **query** hook for `GET .../preview-rerun-from-step?targetStep=X`, keyed under `workflowKeys.detail(runId)` prefix, `enabled` only while the rerun dialog is open and a `targetStep` is chosen. Returns `PreviewRerunFromStepResponse`. Read-only → NO Idempotency-Key.
  - [x] Do NOT build `useReconcileWorkflow`/`useClassifyFailure` here — those belong to stories 4.23/4.24 (which own the reconcile/classify mutations). See "Reconcile / Classify delegation" in Dev Notes.
  - [x] Field-only structured logging on submit/error for each mutation (mirror `RecoveryDecisionBarContainer`'s `recovery.retrySubmit`/`recovery.retryError`): log the response `currentState` on success + the stable ProblemDetails `code` + transport flag on error. NEVER `reasonText`/ids/actor (T-LOG-PII).
- [x] **Task 4 — FE: extend `ApprovalDecisionBar` `recovery_operator` render (AC1, AC2, AC3, AC5, AC6, AC9, AC10, AC11)**
  - [x] Replace the retry-only `renderRecoveryOperator()` with a multi-action renderer driven by `view.actions` ∩ safety model: one primary (`resolveRecoveryPrimaryAction`) + subordinate secondaries, each labeled with its explicit verb and safety affix. Preserve the existing `success`/`submitting`/`error`/`disabled(view-only)` state branches and the kept-alive success summary.
  - [x] Add new `on*` callback props to `ApprovalDecisionBarProps`: `onResume?`, `onRerunFromStep?(vars: { targetStep; reasonText })`, `onPause?`, `onReconcile?`, `onClassifyFailure?`. Keep the props flat/optional like the existing `onRetry`/`onTakeover`.
  - [x] Wire the resume + pause `ConfirmationDialog`s and the rerun `RationaleCaptureDialog` (fields: `targetStep` select + required `reasonText` textarea; render the preview section from `usePreviewRerunFromStep` inside the dialog `children`). Confirm buttons call the respective `on*` callbacks. Use `intent` per AC: resume `warning`/`info`, pause `warning`, rerun `danger`.
  - [x] Render reconcile + classify as entry-point buttons that invoke `onReconcile`/`onClassifyFailure` (no inline dialog — delegated to 4.23/4.24).
  - [x] Post-submit summary (AC9): reuse the `recovery` chip + a `DecisionSummary`-style block showing resulting state + correlationId (extend `DecisionSummary.decision` union or add a recovery-specific summary path). Single polite live-region announcement per action lifecycle (extend the `recovery_operator` branch of `announcementText`).
- [x] **Task 5 — FE: extend `RecoveryDecisionBarContainer` (AC1, AC2, AC8, AC10, AC11)**
  - [x] Keep `useAllowedActions(workflowRunId, RECOVERY_OPERATOR_ROLE)` + `useWorkflowDetail`. Add `useFailureDiagnostics(workflowRunId)` for the safety ranking; compute `safetyByToken` via `resolveRecoverySafety`. Build the `ApprovalDecisionView` with the full `actions` list + pass the safety model + all `on*` handlers.
  - [x] Instantiate/accept the new mutations (retry stays; add resume/rerun/pause — accept them as optional props for the route to lift, defaulting to internal instances, exactly like the existing `retry?` prop). Map the aggregate mutation status (pending/success/error) to `ApprovalMutationState`, keyed to whichever action is in flight.
  - [x] Wire `onReconcile`/`onClassifyFailure` to the seam (Open Question OQ-2 decides the interim behavior). Refresh handler refetches allowed-actions + detail + diagnostics and resets in-flight mutations.
- [x] **Task 6 — FE: widen the `WorkflowDecisionBar` selector (AC1, AC9)**
  - [x] Currently `showRecovery = data?.currentState === 'Failed' || retry.isPending || retry.isSuccess`. Widen to also select the recovery bar for `Paused` (so `resume_workflow`/`reconcile_conflict` render) and keep it mounted through resume/rerun/pause settling (`|| resume.isPending || resume.isSuccess || rerun.* || pause.*`). Hoist the new mutation instances here (like `retry`) so a post-action `currentState` flip (e.g. `Paused → Executing` on resume, `Failed → Investigating` on rerun) does not unmount the bar and tear down its success summary + announcement.
  - [x] Confirm the three selector predicates stay disjoint (`WaitingForReview` vs recovery-states vs everything-else) — `Paused` and `Failed` are both recovery; no collision with `implementation_review`/`spec_approval`.
- [x] **Task 7 — FE: confirmation catalog entries (AC3, AC5, AC6)**
  - [x] Add `CONFIRMATION_CATALOG` entries `resumeRun` (`intent: 'warning'`/`'info'`), `pauseRun` (`intent: 'warning'`), `rerunFromStep` (`intent: 'danger'`) with the exact consequence copy from AC3/AC5/AC6. Extend the `ConfirmationActionId` union + `CONFIRMATION_ACTION_IDS` list + the catalog test (which iterates all ids).
- [x] **Task 8 — FE: tests (AC13)**
  - [x] `RecoveryDecisionBarContainer.test.tsx`: full action set renders only when allowed (MSW allowed-actions fixtures for `Failed` = retry/rerun/pause/classify and `Paused` = resume; `reconcile_conflict` present variant); single primary per safety fixture; each dialog's consequence + severity; rerun preview fetch renders superseded/invalidated ids; mutation sends `Idempotency-Key` + `role`; typed-conflict → stale CTA; post-submit summary persists across the state flip; reconcile/classify buttons invoke their callback spies; keyboard nav; `expectNoA11yViolations`.
  - [x] `approvalDecisionView.test.ts`: coercion of the 5 new tokens; safety join + short-form mapping + fallbacks; primary resolution tie-breaks.
  - [x] `WorkflowDecisionBar.test.tsx`: `Paused` selects the recovery bar; kept-alive through resume/rerun/pause settling.
  - [x] Run the REAL frontend build (`npm run build` — `tsc -b` typechecks test files that `tsc --noEmit` misses) + eslint + vitest before claiming green.
- [x] **Logging instrumentation** (cross-cutting; required on every story)
  - [x] Backend: `INFO` on `previewRerunFromStep` entry/exit with `workflowRunId` (sanitized) + `targetStep` + result counts; `WARN`/typed on `INVALID_RERUN_TARGET_STEP`/`ILLEGAL_TRANSITION`; NEVER log payloads or ids beyond public ids. Follow the dominant controller convention (`MdcKeys.sanitizeForLog`).
  - [x] Frontend: field-only structured console per Task 3 (`recovery.resumeSubmit`/`recovery.resumeError`, `recovery.rerunSubmit`/`recovery.rerunError`, `recovery.pauseSubmit`/`recovery.pauseError`, `recovery.previewLoadError`, `recovery.allowedActionsLoadError`). Assert the exact object keys in a focused test to prove no PII leaks (mirror the baseline container's console-key assertions).
  - [x] Use parameterized logging; levels per the standard below; carry `correlationId`/`workflowRunId` where available; never log `reasonText`, `taxonomyValue` free text, tokens, or PII.
...
- OpenAPI regen: `OpenApiSnapshotContractTest` with `-Dopenapi.snapshot.write=true` (writes+fail()s), reviewed the +107-line additive diff (1 path + `PreviewRerunFromStepResponse` schema), re-ran byte-identical, then `npm run generate-api` (+85 lines in `schema.d.ts`, both id lists `string[]` REQUIRED). Em-dash verified (no mojibake); the `targetStep` nullable+enum query param renders `"type":"null"` exactly like the established `actorRole` pattern.
- `{argLine}` trap: the OpenAPI regen only wrote via the lifecycle `verify` phase + `-Djacoco.skip=true` (a direct `failsafe:integration-test` goal crashed on `@{argLine}`, and Tests-run:0).
- Full backend `verify` surfaced 2 failures in `DoctorProbeAdapterTest` + `LocalRunnerLogStoreTest` (filesystem-probe tests, unrelated to this story); both pass in isolation (37/0, 7/0) — confirmed Windows file-lock flakes from concurrent Surefire forks, NOT regressions.
- FE `tsc --noEmit` was clean, but the real `npm run build` (`tsc -b`, which typechecks test files) caught two `as` casts in `approvalDecisionView.test.ts` (diagnostics fixture + a non-DecisionAction literal) → fixed with `as unknown as`.

### Completion Notes List

Full-stack story (one net-new backend read endpoint + substantial FE activation). AS BUILT:

- **Backend (Task 1):** `RecoveryService.previewRerunFromStep(runId, targetStep)` — NON-MUTATING, reuses the pure `resolveTargetState` (→ `INVALID_RERUN_TARGET_STEP`), `RERUN_SOURCE_STATES` gate (→ `ILLEGAL_TRANSITION`, OQ-1 default), `resolveSupersededArtifactIds`, and a NEW read-only `ApprovalService.findCurrentApprovalId` (the read half of `invalidateCurrentApproval`, keeping the approval read inside the approval package — no `ApprovalReadPort` leak into RecoveryService). New `RerunFromStepPreviewResult` record + `PreviewRerunFromStepResponse` DTO (both id lists REQUIRED never-null) + `WorkflowController.previewRerunFromStep` GET (copies the `getAllowedActions` shape: no Idempotency-Key/actor/role; `targetStep` a plain `@RequestParam(required=false)` String with NO `@NotBlank`/`@Pattern` so the typed code is not masked; `.strip()` normalized). NO migration, NO new DomainErrorCode/WorkflowEventType. AC12 scope-lift confirmed merged (4.28).
- **FE view-model (Task 2):** 5 long-form tokens added to `DecisionAction` + `KNOWN_ACTIONS`; `RecoverySafetyLevel` + `resolveRecoverySafety` (token→SHORT-FORM-diagnostics-actionType map `{retry→retry, pause_workflow→pause, reconcile_conflict→reconcile, classify_failure→classify_failure}` + static fallbacks `rerun_from_step→risky`, `resume_workflow→safe`, else `caution`) + `buildRecoverySafetyByToken` + `resolveRecoveryPrimaryAction` (single safe action via tie-break `retry>resume>reconcile>rerun>pause>classify`) + `hasRecoveryAction`. `CONSEQUENCE_HINTS.recovery_operator` extended.
- **FE hooks (Task 3):** `useResumeWorkflow`/`useRerunFromStep`/`usePauseWorkflow` (role-in-body via `RECOVERY_OPERATOR_ROLE`, NOT retry's actorIdentity/actorType shape) + `usePreviewRerunFromStep` query (keyed under `workflowKeys.rerunPreview(runId, targetStep)`, enabled only while dialog open).
- **FE bar (Task 4):** rewrote `renderRecoveryOperator` — one safety-ranked primary (`GovernedButton priority=primary`) + subordinate secondaries each with a non-color-only safety affix; resume/pause `ConfirmationDialog`s (pause captures a REQUIRED reason like takeover, since the endpoint raises `MISSING_REASON_TEXT`); rerun `RationaleCaptureDialog` with a bar-owned `targetStep` select (in `children`, so the preview reacts to it — reasonText stays a stable module-const field) + the "Show what will be superseded" preview section; reconcile/classify entry-point seams (`onReconcile`/`onClassifyFailure`) render disabled+explained when no handler (OQ-2 seam-only default); post-submit `DecisionSummary` for resume/rerun/pause (retry keeps its bespoke chip); the `announcementText` recovery branch resolves the specific action via `pendingRecoveryAction`/`lastDecision`. `resolveApprovalBarState` recovery branch broadened from `canRetry` → `hasRecoveryAction`.
- **FE container (Task 5):** added `useFailureDiagnostics` + `buildRecoverySafetyByToken`; wired resume/rerun/pause mutations (optional props defaulting internal, like retry) + the preview query (driven by the bar's `onRerunPreviewRequest`); aggregate mutation status; `lastDecision` set on landed resume/rerun/pause; field-only logging (`recovery.<action>Submit`/`Error`, `recovery.previewLoadError`, `recovery.allowedActionsLoadError`). reconcile/classify handlers intentionally NOT wired (OQ-2 → 4.23/4.24 attach later).
- **FE selector (Task 6):** `WorkflowDecisionBar.showRecovery` widened to `Failed || Paused` + kept-alive through resume/rerun/pause settling; new mutations hoisted here so the post-action state flip does not unmount the bar. Predicates stay disjoint.
- **FE catalog (Task 7):** `resumeRun` (info), `pauseRun` (warning), `rerunFromStep` (danger) added to `CONFIRMATION_CATALOG` + `ConfirmationActionId` + `CONFIRMATION_ACTION_IDS` + the catalog test's expected set.

**Decisions:** OQ-1 → 409 `ILLEGAL_TRANSITION` (preview mirrors the mutation). OQ-2 → seam-only default (reconcile/classify render gated-but-disabled+explained until 4.23/4.24). OQ-3 → `Paused` `resume_workflow` uses the static `safe` fallback (accepted; no backend recommendation added).

**Verification (all green):** Backend — RecoveryServiceRerunFromStepTest 20/0 (6 new zero-write preview cases), PreviewRerunFromStepEndpointContractTest 7/0, OpenApiSnapshot byte-identical + operationId pin, ArchitectureBoundaryTest 63/0, foundation-gate 71/0. Frontend — full vitest 1365/0 (approvalDecisionView + ApprovalDecisionBar.recovery + RecoveryDecisionBarContainer.full + WorkflowDecisionBar.paused new suites; axe-core clean), `npm run build` (tsc -b + vite) green, eslint 0, check:a11y 4/0, check:api in sync.

### File List

**Backend (modified):**
- `deliveryline-backend/src/main/java/org/dradgo/application/recovery/RecoveryService.java` — `previewRerunFromStep`
- `deliveryline-backend/src/main/java/org/dradgo/application/approval/ApprovalService.java` — `findCurrentApprovalId`
- `deliveryline-backend/src/main/java/org/dradgo/adapters/rest/WorkflowController.java` — `previewRerunFromStep` endpoint
- `deliveryline-backend/src/main/resources/openapi/openapi.json` — regenerated (+ preview path/schema)
- `deliveryline-backend/src/test/java/org/dradgo/application/recovery/RecoveryServiceRerunFromStepTest.java` — 6 preview zero-write tests
- `deliveryline-backend/src/test/java/org/dradgo/adapters/rest/OpenApiSnapshotContractTest.java` — previewRerunFromStep operationId assertion

**Backend (new):**
- `deliveryline-backend/src/main/java/org/dradgo/application/recovery/RerunFromStepPreviewResult.java`
- `deliveryline-backend/src/main/java/org/dradgo/adapters/rest/PreviewRerunFromStepResponse.java`
- `deliveryline-backend/src/test/java/org/dradgo/adapters/rest/PreviewRerunFromStepEndpointContractTest.java`

**Frontend (modified):**
- `deliveryline-frontend/src/lib/api/schema.d.ts` — regenerated
- `deliveryline-frontend/src/features/workflows/approvalDecisionView.ts` — tokens + safety model + DecisionSummary union
- `deliveryline-frontend/src/features/workflows/components/ApprovalDecisionBar.tsx` — multi-action recovery render
- `deliveryline-frontend/src/features/workflows/components/RecoveryDecisionBarContainer.tsx` — full activation
- `deliveryline-frontend/src/features/workflows/components/WorkflowDecisionBar.tsx` — Paused + hoisted mutations
- `deliveryline-frontend/src/lib/overlays/confirmationCatalog.ts` — resume/pause/rerun entries
- `deliveryline-frontend/src/lib/queryKeys/workflowKeys.ts` — `rerunPreview` key
- `deliveryline-frontend/src/lib/a11y/announcements.ts` — recovery resume/rerun/pause strings
- `deliveryline-frontend/src/features/workflows/approvalDecisionView.test.ts` — 4.22 unit tests
- `deliveryline-frontend/src/features/workflows/components/ApprovalDecisionBar.test.tsx` — updated 3.30 recovery tests for the safety model
- `deliveryline-frontend/src/lib/overlays/__tests__/confirmationCatalog.test.ts` — expected-set update

**Frontend (new):**
- `deliveryline-frontend/src/features/workflows/hooks/useResumeWorkflow.ts`
- `deliveryline-frontend/src/features/workflows/hooks/useRerunFromStep.ts`
- `deliveryline-frontend/src/features/workflows/hooks/usePauseWorkflow.ts`
- `deliveryline-frontend/src/features/workflows/hooks/usePreviewRerunFromStep.ts`
- `deliveryline-frontend/src/features/workflows/components/ApprovalDecisionBar.recovery.test.tsx`
- `deliveryline-frontend/src/features/workflows/components/RecoveryDecisionBarContainer.full.test.tsx`
- `deliveryline-frontend/src/features/workflows/components/WorkflowDecisionBar.paused.test.tsx`

### Change Log

- 2026-07-15 — Story 4.22 implemented (`ready-for-dev → review`). Decision Bar `recovery_operator` mode FULLY ACTIVATED beyond the 3.30 retry baseline: resume / reconcile (seam) / rerun-from-step (+ non-mutating preview) / pause / classify-failure (seam), safety-ranked with a single primary, each guarded by consequence-appropriate confirmation. One net-new backend read endpoint (`GET .../preview-rerun-from-step`).

### Review Findings

_Code review 2026-07-15 (bmad-code-review, 3 adversarial layers: Blind Hunter / Edge Case Hunter / Acceptance Auditor). All 13 ACs + OQ-1/2/3 verified satisfied by the Acceptance Auditor. 10 findings dismissed as noise/false-positive (incl. the OpenAPI `type:null` schema — verified correct `actorRole`-pattern codegen; and an alleged preview/mutation gate divergence — verified identical `RERUN_SOURCE_STATES` gate on both paths)._

- [x] [Review][Patch] (resolved from Decision) Add a `retry → safe` static fallback so retry stays deterministically primary while `useFailureDiagnostics` is loading or errored. Currently `retry`/`pause_workflow`/`classify_failure` have no `RECOVERY_STATIC_SAFETY` entry, so during the diagnostics-load window — and *permanently* if the diagnostics GET fails — `resolveRecoverySafety` returns `caution`, `resolveRecoveryPrimaryAction` returns `null`, and the bar renders ZERO primary controls (AC2 "exactly one primary" violated) with `retry` mislabeled "Caution" despite the backend ranking it `safe`. **FIXED:** added `retry: 'safe'` to `RECOVERY_STATIC_SAFETY` (a present diagnostics rank still wins via the join); test updated + new override-floor test. [`approvalDecisionView.ts:599-631`] (source: edge+auditor)
- [x] [Review][Patch] (resolved from Decision) Add a refresh/next-action CTA to the pause-success branch so the now-valid Resume is reachable. After a successful pause (`Failed → Paused`, bar kept mounted via `pause.isSuccess`), the aggregate `mutation.status` pins to `success` and the bar renders only the persisted summary with NO Refresh CTA (unlike the `error`/`stale` branches), stranding the `resume_workflow` action until an external refetch. **FIXED:** the pause-success summary (`decision === 'paused'`) now renders a "Refresh to continue" button wired to `onRefresh`. [`ApprovalDecisionBar.tsx:849-870`] (source: edge)
- [x] [Review][Patch] `failure-diagnostics` query load failure is not logged — the project-wide Logging Requirements name `failure-diagnostics` explicitly, but only `recovery.allowedActionsLoadError` + `recovery.previewLoadError` exist; a diagnostics read failure is invisible (safety silently falls back to caution). **FIXED:** added a `recovery.diagnosticsLoadError` warn effect on `diagnosticsQuery.isError` (does not force the bar's error state — safety still falls back). [`RecoveryDecisionBarContainer.tsx`] (source: auditor)
- [x] [Review][Patch] `preview-rerun-from-step` `400 INVALID_ID_PREFIX` path has no endpoint test — the controller advertises this branch in its OpenAPI `400` description and relies on `PublicIdPrefixes.require` throwing a typed `DomainException` to avoid a masked 500, but no contract test sends a malformed `workflowRunId`. **FIXED:** added `malformedRunIdSurfacesTypedInvalidIdPrefix400` (Failsafe 8/0/0). [`PreviewRerunFromStepEndpointContractTest.java`] (source: blind)
- [x] [Review][Defer] `pause_workflow` is unreachable in the mid-flight running states that advertise it (Investigating/Executing/WaitingForSpecApproval/WaitingForLintApproval/WaitingForManualExecution) — Task 6 mounts the recovery bar only for `Failed`/`Paused`, and the spec-approval/implementation-review bars render no pause control [`WorkflowDecisionBar.tsx:73-83`] — deferred, pre-existing scope gap (those states never rendered a pause control; out of this story's `recovery_operator` scope). (source: edge)
- [x] [Review][Defer] Safe-seam primary demotion has no fallthrough to the next safe non-seam action — a safe `reconcile_conflict` (unwired seam, tie-break rank 2) sets `primary = null` without re-selecting a co-present safe `pause_workflow`, yielding zero primary despite a safe actionable action [`ApprovalDecisionBar.tsx:923-928`] — deferred, latent only (masked today because `RecommendationService`'s drift rule prevents two `safe` actions co-occurring). (source: edge)
- [x] [Review][Defer] `usePreviewRerunFromStep` uses `staleTime: STALE_TIME.detail` — a reopened rerun dialog can render a stale "what will be superseded" blast-radius if artifacts/approval change via any path that is not a `detail(id)` invalidation (background producer sweep, sibling tab, cross-run reconciliation) [`usePreviewRerunFromStep.ts`] — deferred, low (advisory preview re-checked by the mutation under lock; a shorter/zero stale time on this dialog-gated read would harden it). (source: blind)
