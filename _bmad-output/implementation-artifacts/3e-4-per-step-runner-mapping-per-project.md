...
3. **Given** the `Project` aggregate, **Then** it gains a `Map<ProjectRunnerStep, RunnerKind> stepRunnerKinds` component (empty map = no per-step mapping; null-safe). The 6 `new Project(...)` sites (`ProjectManagementService`, `ProjectController`, `ProjectEntityMapper`, `ProjectConnectorResolver` helper, `DefaultProjectSeeder`, tests — per 3d-3) are updated; the default project + seeder pass an empty map (parity).
...
- [ ] **Task 2 — `project_runner_kinds` table** (AC: 1)
  - [ ] Flyway (next free version — V20 = manual execution; confirm head at impl time, coordinate with 3d-7 if it took V21): `create table project_runner_kinds (project_id text not null references projects(public_id), step text not null, runner_kind text not null, primary key (project_id, step), constraint ck_project_runner_kinds_step check (step in ('spec','implementationPlan','prOutput')), constraint ck_project_runner_kinds_kind check (runner_kind in ('codex','claude','manual')))`. Additive, replay-safe.
  - [ ] `FlywaySchemaContractTest` — add the new table to expectations.

- [ ] **Task 3 — `Project` model + persistence fan-out** (AC: 3)
  - [ ] `domain/project/Project.java` — add `Map<ProjectRunnerStep, RunnerKind> stepRunnerKinds` (trailing component; defensive-copy to unmodifiable empty if null). **Fan-out (6 `new Project(...)` sites, per [[story-3d-3-manual-runner-kind-and-waiting-for-manual-execution-reconciliations]]):** `ProjectManagementService`, `ProjectController`, `ProjectEntityMapper`, `ProjectConnectorResolver` helper, `DefaultProjectSeeder` (empty map — parity), tests.
...
- `Project` fan-out (6 sites) — `ProjectManagementService`, `ProjectController`, `ProjectEntityMapper`, `ProjectConnectorResolver`, `DefaultProjectSeeder`, tests.
