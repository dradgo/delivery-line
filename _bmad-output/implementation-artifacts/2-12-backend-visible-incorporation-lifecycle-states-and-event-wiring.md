...
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — dev-story start + completion entries; 2.12 flipped `ready-for-dev -> in-progress -> review`.
- `_bmad-output/implementation-artifacts/2-12-backend-visible-incorporation-lifecycle-states-and-event-wiring.md` — Status flipped to `review`; top-level Tasks/Subtasks checkboxes flipped; this Dev Agent Record populated.
