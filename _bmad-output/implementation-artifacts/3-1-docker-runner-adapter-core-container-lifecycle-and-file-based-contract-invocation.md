...
- [ ] [Review][Patch] Docker lifecycle acceptance coverage is incomplete [deliveryline-backend/src/test/java/org/dradgo/adapters/runner/DockerRunnerAdapterContainerLifecycleIT.java:107] — AC10 calls for happy path, result-bearing non-zero exit, missing-result failures, invalid/schema-mismatch broker classification, image override, network probe, exact mount modes, labels, cancel, and broker handoff. The current IT has only two shape tests and no `RunnerBrokerDockerHandoffContractTest`. Add the specified scripted Docker/gateway/broker tests or explicitly reduce the AC.
...
- [ ] [Review][Patch] Required Docker adapter logging contract is missing [deliveryline-backend/src/test/java/org/dradgo/adapters/runner/DockerRunnerAdapterUnitTest.java:72] — the story requires `DockerRunnerAdapterLoggingContractTest` with branch-level log assertions; current tests only assert behavior. Add list-appender tests for dispatch, poll, cancel, failures, and image-tag redaction.
