...
**Story list (7 stories):** 3f-1 adapter `createSubticket` capability; 3f-2 parent→child lineage + non-terminal `Split` state; 3f-3 run-dependency graph + `WaitingForDependencies` state + release resolver; 3f-4 LLM split-proposal channel + three-action feedback loop; 3f-5 commit fan-out (subtickets/internal-only children + lineage + dependency edges + gated dispatch + parent decomposition); 3f-6 project references + queue filter; 3f-7 recursive split + split-parent completion rollup (`Split → Completed` rollup resolver, cross-split dependency unblocking, soft depth cap). Detailed reconciled implementation stories live at `{implementation_artifacts}/3f-1..3f-7-...md`.
