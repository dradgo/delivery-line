...
- **Scope amendment (2026-06-21, Epic 3d):** per-step execution control and operator observability are added — a per-project **advisory reviewer model** (a second LLM reviews each step's output; verdict is advisory now, gating-capable per project later), a first-class **manual execution mode** (a `manual` runner kind that emits the context bundle, parks the run in `WaitingForManualExecution`, and accepts an operator-produced artifact through the same validation/review pipeline — the supported path for agents whose unattended/headless auth is unavailable), live and historical container-log viewing per step, soft **hide/archive** of obsolete executions (audit history preserved per FR47 — never hard-deleted), post-execution **provider usage/limit status** (5-hour/weekly windows where the provider exposes them), and a **read-only diagnostic console** into a running runner container. The console **narrows but does not remove** the runner sandbox posture: it is read-only, attaches only to a live runner, and every console session is recorded in governed history; write-capable shells, host shells, and ungoverned access remain out of scope. No change to the multi-user-auth / RBAC / tenant-isolation boundary. See `docs/adr/0024`–`0027`.
...
- **Data model:** Use explicit relational tables for core concepts: `workflow_runs`, `workflow_events`, `artifacts`, `artifact_operations`, `approvals`, `runner_executions`, `integration_links`, and `recovery_actions`. Epic 3c adds `projects` and `project_credentials` (Flyway V14) and a `project_id` foreign key on `workflow_runs` (and `integration_links`) so every run is scoped to one configured project. Epic 3d adds per-step reviewer configuration and advisory-verdict records, a `WaitingForManualExecution` run state and a `manual` runner kind, per-execution provider usage/limit snapshots, console-session audit records, and `archived_at`/hide markers for obsolete-execution soft-deletion — all additive, with append-only audit history preserved. JSON is allowed only for bounded metadata, not as the primary domain model.
