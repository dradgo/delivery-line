...
| `backend-contract-tests` | ubuntu | Linux/Docker-backed backend `verify` tier — runs the full `verify` lifecycle on Linux: Surefire unit tests plus Failsafe ArchUnit + Testcontainers + contract + integration tests (Testcontainers needs Docker), picked up by `@Tag` or class-name suffix. Also enforces the JaCoCo coverage threshold gate (`jacoco-check`). AC7 — ArchUnit fails loudly with offending class + rule + remediation hint. Also runs the OpenAPI snapshot drift check (no-op until story 6.9). | 4-8 min | `ArchitectureBoundaryTest` rule violation, Testcontainers IT failure, contract-test failure, (future) OpenAPI snapshot drift, or a JaCoCo `LINE`/`BRANCH` coverage-threshold breach. |
...
JaCoCo coverage HTML/XML/CSV is uploaded as `backend-coverage-jacoco`. Story 2.32 wired the
**coverage threshold gate**: the `jacoco-check` execution, bound to `verify` in
`deliveryline-backend/pom.xml`, fails the build when backend `LINE` or `BRANCH` coverage drops
below the committed floors. The gate evaluates the **merged** execution data
(`jacoco-merged.exec`), so the report and the gate cover both the Surefire unit tier and the
Failsafe integration/contract tier. The committed floors were calibrated against the full
`verify` lifecycle measured on the CI platform; the threshold values and the
measured-minus-headroom rationale are documented in `deliveryline-backend/README.md`.
