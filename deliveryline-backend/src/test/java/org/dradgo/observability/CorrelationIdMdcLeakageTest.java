...
/**
 * Pins AC2 of story 1.19: a CLI command MUST {@code MDC.remove(correlationId)} in a {@code finally}
 * block, so two sequential invocations cannot share or leak correlation state. Independent of the
 * {@code WorkflowCommandsTest} family — that suite verifies user-visible stdout; this suite
 * verifies the structured-log surface.
 */
class CorrelationIdMdcLeakageTest {
  ...
  void setUp() {
    ...
    when(service.submit(any(SubmitWorkflowCommand.class)))
        .thenAnswer(
            invocation -> {
              SubmitWorkflowCommand command = invocation.getArgument(0);
              return new SubmitWorkflowResult(
                  "run_test-aaaaa", WorkflowState.INBOX, command.correlationId());
            });
    commands =
