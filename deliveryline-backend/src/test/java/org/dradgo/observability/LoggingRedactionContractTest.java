...
/**
 * Story 1.19 AC5: every log message is routed through {@link RedactionPolicyService} via the
 * {@link RedactingMessageConverter} (the production layout converter registered in
 * {@code logback-spring.xml} as the {@code redactedMsg} conversion word). This adversarial
 * sweep emits each {@code redaction-fixtures/*.{txt,json,env,pem,yaml}} payload through SLF4J
 * as {@code log.info("payload {}", fixture)}, captures the {@code ILoggingEvent} via
 * {@link ListAppender}, then runs the captured event through {@code RedactingMessageConverter}
 * — i.e. it exercises the same converter the production XML wires.
 *
 * <p>Reworked under P7 of the story 1.19 review: the previous version called
 * {@code RedactionLayoutHolder.redact(raw)} directly, so a regression in the converter wiring
 * would have been invisible.
 */
class LoggingRedactionContractTest {

	private static final Path FIXTURE_DIR = Paths.get("src", "test", "resources", "redaction-fixtures");
	...
	private void assertFixtureRedacted(Path fixture) throws Exception {
		...
		for (String line : lines) {
			...
			if (looksSecret(trimmed) && rendered.contains(trimmed)) {
				throw new AssertionError("Fixture " + fixture.getFileName()
