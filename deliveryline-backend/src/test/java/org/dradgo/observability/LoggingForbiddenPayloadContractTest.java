 ...
 * "Consistency Drift Prevention" rule. Each surface MUST log only its permitted allow-listed fields
 * — never the full context bundle / Authorization header / file contents / artifact bytes.
 *
 ...
 * names the production surfaces use, routes the captured event through {@link
 * RedactingMessageConverter} (the same converter wired into {@code logback-spring.xml}), and
 * asserts the rendered output does NOT contain the raw forbidden substring. This pins the AC5
 * safety net for the AC6 surfaces — call-site discipline must be re-verified by the follow-up audit
 * recorded under D5 of the same review.
 */
class LoggingForbiddenPayloadContractTest {
  ...
  void runnerBrokerDispatchDoesNotEmitBearerTokenInPayloadAfterRedaction() {
    ...
    emit("org.dradgo.application.runner.RunnerBroker", "dispatch payload bundle={}", forbidden);
  ...
  void doctorProbeAdapterDoesNotEmitEnvVarSecretsAfterRedaction() {
    ...
    emit("org.dradgo.adapters.diagnostics.DoctorProbeAdapter", "probe env {}", forbidden);
  ...
  void localArtifactStoreDoesNotEmitEmbeddedSecretInArtifactPayloadAfterRedaction() {
    ...
    emit("org.dradgo.adapters.files.LocalArtifactStore", "artifact payload={}", forbidden);
  ...
  private void assertRenderedScrubsForbiddenSubstring(
      ListAppender<ILoggingEvent> appender, String fullPayload, String forbiddenSubstring) {
    ...
    for (ILoggingEvent event : appender.list) {
      ...
      assertThat(rendered)
          .as(
              "post-redaction rendered log line must NOT contain forbidden substring "
                  + "["
                  + forbiddenSubstring
                  + "] for event "
