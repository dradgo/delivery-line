...
class ArchitectureDiagnosticMetaTest {
	...
	void workflowStateMutationRuleFailsWhenNonTransitionCodeCallsTheEntitySetter() {
		...
		AssertionError error = assertThrows(
			AssertionError.class,
			() -> ArchitectureRuleCatalog.ONLY_WORKFLOW_TRANSITION_SERVICE_MAY_MUTATE_WORKFLOW_STATE.check(classes));

		assertTrue(error.getMessage().contains("workflow state changes must go through WorkflowTransitionService"));
	...
	void artifactWriteRuleFailsWhenNonArtifactApplicationCodeCallsTheStoreDirectly() {
		...
		AssertionError error = assertThrows(
			AssertionError.class,
			() -> ArchitectureRuleCatalog.ARTIFACT_WRITES_MUST_GO_THROUGH_ARTIFACT_OPERATION_SERVICE.check(classes));
