...
final class ArchitectureRuleCatalog {
	...
	private static final String PERSISTENCE_REPOSITORY_PACKAGE = "org.dradgo.adapters.persistence.repository..";
	private static final String PERSISTENCE_MAPPER_PACKAGE = "org.dradgo.adapters.persistence.mapper..";

	private static final Pattern SENSITIVE_PATTERN_NAME = Pattern.compile(
		".*(SECRET|TOKEN|PASSWORD|AUTHORIZATION|CREDENTIAL|PRIVATE_KEY|API_KEY).*");
	private static final List<String> SENSITIVE_PREDICATE_NAMES = List.of(
		"redactIfSensitive",
		"looksSecretLikeKey",
		"looksHighEntropy",
		"alreadyRedacted");

	static final ArchRule LAYERED_BOUNDARIES = namedRule(
	...
	static final ArchRule APPLICATION_PACKAGE_MUST_EXIST = namedRule(
	...
	static final ArchRule ADAPTERS_PACKAGE_MUST_EXIST = namedRule(
	...
	static final ArchRule ADAPTER_SLICES_MUST_BE_FREE_OF_CYCLES = namedRule(
	...
	static final ArchRule ADAPTER_SLICES_MUST_NOT_DEPEND_ON_EACH_OTHER = namedRule(
	...
	static final ArchRule DOMAIN_MUST_BE_FRAMEWORK_FREE = namedRule(
	...
	static final ArchRule APPLICATION_AND_DOMAIN_MUST_NOT_DEPEND_ON_PERSISTENCE_TYPES = namedRule(
		...
		noClasses().that().resideInAnyPackage(APPLICATION_PACKAGE, DOMAIN_PACKAGE)
			.should().dependOnClassesThat().resideInAnyPackage(
				PERSISTENCE_ENTITY_PACKAGE,
				PERSISTENCE_REPOSITORY_PACKAGE));

	static final ArchRule APPLICATION_MUST_NOT_DEPEND_ON_INFRASTRUCTURE = namedRule(
	...
	static final ArchRule REST_AND_CLI_ADAPTERS_MUST_NOT_TOUCH_PERSISTENCE_OR_EXTERNAL_ADAPTERS = namedRule(
		...
		noClasses().that().resideInAnyPackage(CLI_ADAPTER_PACKAGE, REST_ADAPTER_PACKAGE)
			.should().dependOnClassesThat().resideInAnyPackage(
				PERSISTENCE_ADAPTER_PACKAGE,
				FILES_ADAPTER_PACKAGE,
				RUNNER_ADAPTER_PACKAGE,
	...
	static final ArchRule REST_AND_CLI_ADAPTERS_MUST_NOT_TOUCH_JPA_ENTITIES = namedRule(
	...
	static final ArchRule REST_CONTROLLERS_MUST_BE_NAMED_AS_CONTROLLERS = namedRule(
		...
		classes().that().areAnnotatedWith(RestController.class)
			.should().resideInAPackage(REST_ADAPTER_PACKAGE)
			.andShould(haveSimpleNameEndingWithAny("Controller"))
			.allowEmptyShould(false));

	static final ArchRule REST_CONTROLLER_SUFFIX_REQUIRES_REST_CONTROLLER_ANNOTATION = namedRule(
		...
		classes().that().haveSimpleNameEndingWith("Controller")
			.and().resideInAPackage("org.dradgo..")
			.should().beAnnotatedWith(RestController.class)
			.andShould().resideInAPackage(REST_ADAPTER_PACKAGE));

	static final ArchRule SPRING_SHELL_COMMANDS_MUST_BE_PLURALIZED = namedRule(
		...
		classes().that().areAnnotatedWith(CommandGroup.class)
			.should().resideInAPackage(CLI_ADAPTER_PACKAGE)
			.andShould(haveSimpleNameEndingWithAny("Commands")));

	static final ArchRule SHELL_COMMANDS_SUFFIX_REQUIRES_COMMAND_GROUP_ANNOTATION = namedRule(
		...
		classes().that().haveSimpleNameEndingWith("Commands")
			.and().resideInAPackage("org.dradgo..")
			.should().beAnnotatedWith(CommandGroup.class)
			.andShould().resideInAPackage(CLI_ADAPTER_PACKAGE));

	static final ArchRule APPLICATION_COMMAND_DTOS_MUST_BE_SINGULAR = namedRule(
		...
		classes().that().implement(WorkflowCommand.class)
			.and().areNotInterfaces()
			.should().resideInAPackage(APPLICATION_PACKAGE)
			.andShould(haveSimpleNameEndingWithAny("Command"))
			.allowEmptyShould(false));

	static final ArchRule PERSISTENCE_ENTITIES_MUST_BE_NAMED_AS_ENTITIES = namedRule(
		...
		classes().that().areAnnotatedWith(Entity.class)
			.should().resideInAPackage(PERSISTENCE_ENTITY_PACKAGE)
			.andShould(haveSimpleNameEndingWithAny("Entity"))
			.allowEmptyShould(false));

	static final ArchRule PERSISTENCE_MAPPERS_MUST_BE_LOCATION_QUALIFIED = namedRule(
		...
		classes().that().resideInAPackage(PERSISTENCE_MAPPER_PACKAGE)
			.should(haveSimpleNameEndingWithAny("Mapper", "Mappers"))
			.allowEmptyShould(false));

	static final ArchRule APPLICATION_SERVICES_MUST_BE_NAMED_AS_SERVICES = namedRule(
	...
	static final ArchRule APPLICATION_RESULTS_MUST_USE_RESULT_OR_OUTCOME_SUFFIX = namedRule(
		...
		classes().that().implement(DomainResult.class)
			.and().areNotInterfaces()
			.should(haveSimpleNameEndingWithAny("Result", "Outcome"))
			.allowEmptyShould(false));

	static final ArchRule ONLY_WORKFLOW_TRANSITION_SERVICE_MAY_MUTATE_WORKFLOW_STATE = namedRule(
		...
		noClasses().that().doNotHaveFullyQualifiedName(WorkflowTransitionService.class.getName())
			.should().callMethod(WorkflowRunEntity.class, "setCurrentState", WorkflowState.class)
			.orShould().callMethod(WorkflowRunStatePort.class, "updateCurrentState", String.class, WorkflowState.class, Long.class));

	static final ArchRule ARTIFACT_WRITES_MUST_GO_THROUGH_ARTIFACT_OPERATION_SERVICE = namedRule(
	...
	private static ArchCondition<JavaClass> notDeclareSensitiveDetectionArtifacts() {
		return new ArchCondition<>("avoid local credential-detection regex catalogs or predicate helpers") {
			@Override
			public void check(JavaClass item, ConditionEvents events) {
				for (JavaField field : item.getFields()) {
					...
					if (patternField && SENSITIVE_PATTERN_NAME.matcher(field.getName()).matches()) {
						events.add(SimpleConditionEvent.violated(
							field,
							item.getName() + " declares sensitive regex field " + field.getName()
								+ " outside application.security"));
					}
				}
				for (JavaMethod method : item.getMethods()) {
					String normalized = method.getName().toLowerCase(Locale.ROOT);
					boolean sensitivePredicate = SENSITIVE_PREDICATE_NAMES.stream()
						.map(name -> name.toLowerCase(Locale.ROOT))
						.anyMatch(normalized::equals);
					if (sensitivePredicate) {
						events.add(SimpleConditionEvent.violated(
							method,
							item.getName() + " declares credential-detection helper " + method.getName()
								+ " outside application.security"));
					}
				}
			}
