...
class IntegrationProfileWiringContractTest {
	...
	private static final String[] LINEAR_PROPERTIES = new String[] {
	...
	void underLinearMockProfileMockAdapterLoadsAndLinearRestClientIsAbsent() {
		contextRunner.withPropertyValues("spring.profiles.active=linear-mock").run(context -> {
			assertThat(context).hasNotFailed();
			assertThat(context).hasSingleBean(LinearAdapter.class);
			assertThat(context.getBean(LinearAdapter.class)).isInstanceOf(LinearMockAdapter.class);
	...
	void underLinearRealProfileRealAdapterLoadsAndRestClientIsPresent() {
		contextRunner.withPropertyValues("spring.profiles.active=linear-real").run(context -> {
			assertThat(context).hasNotFailed();
			assertThat(context).hasSingleBean(LinearAdapter.class);
			assertThat(context.getBean(LinearAdapter.class)).isInstanceOf(LinearRealAdapter.class);
	...
	void withBothProfilesActiveContextRefreshFailsFast() {
		contextRunner.withPropertyValues("spring.profiles.active=linear-mock,linear-real").run(context -> {
			assertThat(context).hasFailed();
			assertThat(context.getStartupFailure()).hasStackTraceContaining("Linear profile conflict");
		});
	}
	 ...
	 * {@link LinearPollingHost} so the broader {@code infrastructure.config} package is not pulled
	 * in (which would drag {@code RunnerConfiguration} → {@code RunnerBroker} into the slice).
	 */
	@Configuration
