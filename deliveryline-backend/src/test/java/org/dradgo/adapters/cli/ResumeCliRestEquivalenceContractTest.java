...
/**
 * Story 4.10 AC6 / Reconciliation 4 — CLI/REST equivalence for {@code resume}. Drives the same
 * logical resume through both surfaces — REST {@code POST /{runId}/resume} (header-derived actor +
 * MDC correlation, omitted {@code X-Actor-Identity} → {@code local-operator}) and the Spring Shell
 * {@code deliveryline operator resume} (positional runId, omitted {@code --actor-identity} → {@code
 * local-operator}, {@code --correlation-id}) — then asserts the four positional arguments captured
 * by the mocked {@link RecoveryService#resume} are equal across the two surfaces, with {@link
 * ActorContext} compared by record equality.
 *
 * <p>Unlike {@code CliRestEquivalenceContractTest} (which asserts a captured {@code
 * WorkflowCommand} record), {@link RecoveryService#resume} takes a positional {@code ActorContext},
 * not a command record, so equivalence is asserted argument-by-argument (Reconciliation 4). Lives
 * in the {@code adapters.cli} package so it can use {@link OperatorCommands}' package-private test
 * constructor. The MDC trick + {@code --correlation-id} keeps both captures stamped with the same
 * correlation id.
 */
@WebMvcTest(controllers = WorkflowController.class)
@Import(ApprovalReviewerRoleResolver.class)
class ResumeCliRestEquivalenceContractTest {

  private static final String RUN_ID = "run_resume_equiv_a";
  private static final String IDEMPOTENCY_KEY = "idem-resume-equiv-aaaaaa";
  private static final String CORRELATION_ID = "corr-resume-equiv-a";
  private static final String REASON = "Broker recovered; resuming.";
  ...
  @Test
  void restAndCliProduceIdenticalResumeArguments() throws Exception {
        ...
        .perform(
            post("/api/v1/workflows/{runId}/resume", RUN_ID)
                ...
                .content(
                    ...
                    {
                      "role": "workflow_owner",
                      "reasonText": "Broker recovered; resuming."
                    }
                    """))
    ...
    verify(recoveryService)
        .resume(restRunId.capture(), restKey.capture(), restActor.capture(), restReason.capture());
    ...
    when(cliRecoveryService.resume(any(), any(), any(), any()))
        .thenReturn(
            new ResumeRecoveryResult(
                "rcv_resume_equiv_a",
                "evt_resume_equiv_a",
    ...
    operatorCommands.resume(RUN_ID, REASON, IDEMPOTENCY_KEY, null, CORRELATION_ID, "text", false);
    ...
    verify(cliRecoveryService)
        .resume(cliRunId.capture(), cliKey.capture(), cliActor.capture(), cliReason.capture());
