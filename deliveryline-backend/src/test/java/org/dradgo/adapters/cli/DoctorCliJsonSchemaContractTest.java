...
class DoctorCliJsonSchemaContractTest {
	...
	private static final String SCHEMA_LOCATION = "classpath:schemas/cli/doctor-report.v1.schema.json";
	...
	private DiagnosticsReport allPass() {
		...
		return new DiagnosticsReport(1, OffsetDateTime.parse("2026-05-14T10:00:00Z"),
			...
			List.of(new DiagnosticsCheck("java-version", DiagnosticsStatus.PASS, "Java 21",
				null, null, details)));
	}

	private DiagnosticsReport oneFail() {
		return new DiagnosticsReport(1, OffsetDateTime.parse("2026-05-14T10:01:00Z"),
			DiagnosticsStatus.FAIL,
			List.of(new DiagnosticsCheck("postgres-connectivity", DiagnosticsStatus.FAIL,
	...
	private DiagnosticsReport allSkip() {
		return new DiagnosticsReport(1, OffsetDateTime.parse("2026-05-14T10:02:00Z"),
			DiagnosticsStatus.PASS,
			List.of(
				new DiagnosticsCheck("runner-image-availability", DiagnosticsStatus.SKIP,
					"Runner image availability check populated in story 3.1",
					null, null, Map.of()),
				new DiagnosticsCheck("frontend-asset-presence", DiagnosticsStatus.SKIP,
