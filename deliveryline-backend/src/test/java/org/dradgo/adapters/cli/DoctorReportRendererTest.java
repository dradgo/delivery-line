...
class DoctorReportRendererTest {
	...
	void renderTextEmitsPerCheckLineAndOverall() {
		DiagnosticsReport report = sampleReport(
			new DiagnosticsCheck("java-version", DiagnosticsStatus.PASS, "Java 21", null, null, Map.of()),
			new DiagnosticsCheck("postgres-connectivity", DiagnosticsStatus.FAIL, "Postgres unreachable",
				"Run 'docker compose up -d postgres' and re-check.", "DOCTOR_POSTGRES_UNREACHABLE", Map.of()));
	...
	void renderJsonIncludesSchemaVersionAndCheckShape() throws Exception {
		...
		DiagnosticsReport report = sampleReport(
			new DiagnosticsCheck("java-version", DiagnosticsStatus.PASS, "Java 21", null, null, details));
	...
	void renderJsonRedactsSecretsInDetails() throws Exception {
		...
		DiagnosticsReport report = sampleReport(
			new DiagnosticsCheck("postgres-connectivity", DiagnosticsStatus.FAIL, "boom",
				"Run docker compose up.", "DOCTOR_POSTGRES_UNREACHABLE", details));
	...
	private DiagnosticsReport sampleReport(DiagnosticsCheck... checks) {
		...
		return new DiagnosticsReport(1, OffsetDateTime.parse("2026-05-14T10:00:00Z"), overall, List.of(checks));
	}
