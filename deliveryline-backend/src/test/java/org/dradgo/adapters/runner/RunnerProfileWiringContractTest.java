...
/**
 * Pins AC9 profile-wiring invariant: {@link MockRunnerAdapter} is the default
 * {@link RunnerAdapter} bean (the broker can autowire it without an explicit profile switch),
 * and it is intentionally NOT loaded when the {@code runners.docker} profile is active so the
 * Epic-3 Docker runner adapter can take its slot without a duplicate-bean conflict.
 *
...
class RunnerProfileWiringContractTest {
	...
	private final ApplicationContextRunner contextRunner = new ApplicationContextRunner()
		.withUserConfiguration(MockRunnerStackTestConfiguration.class);
	...
	void mockRunnerAdapterIsExcludedWhenRunnersDockerProfileIsActive() {
		...
		contextRunner.withPropertyValues("spring.profiles.active=runners.docker").run(context -> {
			assertThat(context).hasNotFailed();
	...
	void mockRunnerAdapterStillLoadsUnderExplicitRunnersMockProfile() {
		...
		contextRunner.withPropertyValues("spring.profiles.active=runners.mock").run(context -> {
			assertThat(context).hasNotFailed();
			assertThat(context).hasSingleBean(RunnerAdapter.class);
			assertThat(context.getBean(RunnerAdapter.class)).isInstanceOf(MockRunnerAdapter.class);
		});
	...
	/**
	 * Slice configuration that only declares the mock-runner stack plus the minimal collaborators
	 * the adapter constructor needs. Mockito stubs stand in for the {@link RunnerScratchStore}
	 * port and {@link RunnerProperties} record so the slice does not pull in the persistence
	 * stack.
	 */
	@Configuration
	@org.springframework.context.annotation.ComponentScan(basePackageClasses = MockRunnerAdapter.class)
