...
class IntegrationLoggingContractTest {
	...
	private static final ActorContext ACTOR = new ActorContext("amelia@local", ActorType.HUMAN, "corr-log");

	private final ListAppender<ILoggingEvent> serviceAppender = attachListAppender(IntegrationLinkService.class);
	private final ListAppender<ILoggingEvent> pollingAppender = attachListAppender(LinearPollingHost.class);
	...
	void linkTicketHappyPathEmitsEntryAndSuccessLogsWithContext() {
		...
		IntegrationLinkService service = new IntegrationLinkService(
			port,
			linearAdapter,
			idempotencyService,
			redactionService,
		...
		when(port.insert(any(NewIntegrationLink.class))).thenReturn(sampleLink("ilk_log12345678", RUN_ID));
