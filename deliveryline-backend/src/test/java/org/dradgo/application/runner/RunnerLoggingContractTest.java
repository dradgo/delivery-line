...
class RunnerLoggingContractTest {
	...
	private static final ActorContext ACTOR = new ActorContext("human-pm", ActorType.HUMAN, "corr-log");
	private static final Clock CLOCK = Clock.fixed(Instant.parse("2026-05-12T13:00:00Z"), ZoneOffset.UTC);
	...
	void dispatchHappyPathLogsInfoWithDispatchOkAndCorrelationKeys() {
		...
		ContextBundle bundle = new ContextBundle(RUN_ID, RunnerStage.INVESTIGATION, REX_ID, 1,
			DataClassification.SHAREABLE_REDACTED, "{}".getBytes(StandardCharsets.UTF_8));
		when(contextBundleService.create(any(), any(), any(), eq(1), any(), any(), any())).thenReturn(bundle);
	...
	void onResultArtifactIngestionFailureLogsWarnWithRunnerContractViolationCategory() {
		...
		when(artifactOperationService.recordOperation(any())).thenAnswer(invocation -> {
			...
			ArtifactRecordSnapshot artifact = ArtifactRecordSnapshot.withoutFailureMetadata(
				"art_log0123456789", command.workflowRunId(), command.artifactType(), 1, null,
				DataClassification.SHAREABLE_REDACTED, null, null, null, ArtifactStatus.FAILED, null);
			ArtifactOperationSnapshot op = new ArtifactOperationSnapshot(
				"op_log0123456789", command.workflowRunId(), "art_log0123456789",
				command.operationType().value(), ArtifactOperationStatus.FAILED,
				command.idempotencyKey(), FailureCategory.RUNNER_CONTRACT_VIOLATION, "duplicate",
				OffsetDateTime.now(CLOCK));
			return new RecordArtifactOperationResult(artifact, op, new ArtifactFailureResult(artifact, op));
		});
	...
	private void assertContainsLogAt(Level level, String fragment) {
		...
		assertTrue(found,
			() -> "Expected " + level + " log containing \"" + fragment + "\" but saw: "
