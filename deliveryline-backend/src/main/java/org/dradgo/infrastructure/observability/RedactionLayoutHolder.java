 ...
 *
 * <p>Wired at application startup by {@link RedactionLayoutInitializer}. Log lines emitted before
 * the holder is wired — Spring boot banners during startup, and any log line emitted during
 * shutdown after {@code @PreDestroy} clears the holder — are routed through a fail-closed
 * placeholder ({@code [redaction-pending]}) so credentials in shutdown-window Hikari/JPA logs
 * cannot leak. This is the resolution recorded for D1 in the story 1.19 code review (2026-05-16).
 */
public final class RedactionLayoutHolder {
  ...
  /**
   * Run the supplied raw text through {@link RedactionPolicyService}. On any failure, fall back to
   * a single safe literal so a logging-layer failure can never propagate into the request path. If
   * the service has not been wired yet (cold start) OR has been cleared (shutdown), return {@link
   * #COLD_START} so unredacted secrets can never reach stdout.
   */
  ...
  /**
   * Test-only snapshot of the current service reference so tests can capture-and-restore the holder
   * around their setUp/tearDown, avoiding cross-test pollution when one test overrides the holder.
   * See P16 of the story 1.19 review.
   */
