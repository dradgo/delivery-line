 ...
 * formatted (then redacted) log message. Replaces the default
 * {@link net.logstash.logback.composite.loggingevent.MessageJsonProvider} in the JSON appender
 * configured for the {@code demo} profile of {@code logback-spring.xml}.
 *
 * <p>The provider runs through {@link RedactionLayoutHolder} so the JSON output and the
 * human-readable pattern output share an identical redaction surface — both are pinned by
 * {@code LoggingRedactionContractTest}.
 */
