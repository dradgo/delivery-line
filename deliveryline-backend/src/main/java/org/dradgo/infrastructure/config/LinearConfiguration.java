 ...
 * <ul>
 *   <li>binds {@link LinearProperties} from the {@code deliveryline.linear.*} namespace,</li>
 *   <li>defines the {@code linearRestClient} bean under the {@code linear-real} profile only,</li>
 *   <li>asserts that {@code linear-mock} and {@code linear-real} are mutually exclusive
 *       (mirrors the {@code RunnerConfiguration} fail-fast pattern from story 1.13).</li>
 * </ul>
 *
 * <p>The Linear polling host bean lives in {@link LinearPollingHost} (separate class so the
 * {@code @Scheduled} annotation and the {@link ConditionalOnProperty} gate are co-located with
 * the scheduling concern, not the wiring concern).
 */
@Configuration
...
public class LinearConfiguration {
	...
	/**
	 * Provisions the dedicated {@code linearRestClient} bean used by {@code LinearRealAdapter}.
	 * Only present under {@code linear-real} — the wiring-contract test (Task 7) asserts that no
	 * {@code linearRestClient} bean exists under {@code linear-mock}, which satisfies AC10's
	 * "no network call under linear-mock" guarantee.
	 */
	...
	private static void assertExclusiveLinearProfile(Environment environment) {
		...
		if (mockActive && realActive) {
			throw new IllegalStateException(
				"Linear profile conflict: " + MOCK_PROFILE + " and " + REAL_PROFILE
