 ...
 * </ul>
 *
 * <p>This is a <em>watcher</em> in Epic 1 — it observes ticket updates and refreshes
 * {@code integration_links.last_sync_at} on matching active rows (AC9 "last_sync_at is updated on
 * each successful poll"). The polling loop does <strong>not</strong> create new integration
 * links — ingestion happens via CLI {@code submit} (story 1.15).
 *
 ...
 *       {@code @PostConstruct}) and seeds {@code lastPollAt} from
 *       {@link IntegrationLinkRecordPort#findMaxLastSyncAtForType(String)}. This prevents a JVM
 *       restart from forgetting prior progress (the cursor was previously held in JVM heap only).
 *       When no active linear links exist, the seed falls back to {@code Instant.now(clock)}.</li>
 ...
 *       error.</li>
 *   <li><strong>Per-ticket touch:</strong> Every observed ticket triggers
 *       {@link IntegrationLinkRecordPort#touchLastSyncAtByTypeAndExternalRef(String, String,
 *       Instant)} so the {@code last_sync_at} column on matching active rows reflects the latest
 *       observation — without forcing a {@code sync_status} transition.</li>
 * </ul>
 */
@Component
@Profile("linear-real")
...
public class LinearPollingHost {
	...
	private void pollLinearInternal() {
		...
		for (LinearTicket ticket : tickets) {
			try {
				boolean updated = integrationLinkRecordPort.touchLastSyncAtByTypeAndExternalRef(
				...
				log.warn("linear_real polling_touch_failed ticketRef={} cause={}",
					ticket.ticketRef(), error.getClass().getSimpleName());
				skipped++;
				touchFailures++;
			}
		}
		log.info("linear_real polling_batch since={} count={} oldest={} newest={} touched={} skipped={} touchFailures={}",
			since, tickets.size(), tickets.get(0).updatedAt(), newest.updatedAt(), touched, skipped, touchFailures);
		if (touchFailures > 0) {
			log.warn("linear_real polling_cursor_preserved since={} newest={} touchFailures={}",
				since, newest.updatedAt(), touchFailures);
			return;
		}
