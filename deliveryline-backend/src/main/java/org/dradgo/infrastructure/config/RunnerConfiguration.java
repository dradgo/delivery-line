 ...
 * <ul>
 *   <li>binds {@link RunnerProperties} from the {@code deliveryline.runner.*} namespace,</li>
 *   <li>hosts the {@link Scheduled} bean that drives {@link RunnerBroker#scanForTimeouts()},</li>
 *   <li>hosts the {@code ApplicationReadyEvent} listener that drives
 *       {@link RunnerBroker#recoverOnStartup()},</li>
 *   <li>fails fast at startup if both {@code runners.mock} and {@code runners.docker}
 *       profiles are active.</li>
 * </ul>
 */
@Configuration
...
public class RunnerConfiguration {
	...
	public RunnerConfiguration(RunnerBroker runnerBroker, RunnerProperties runnerProperties, Environment environment) {
	...
	private static void assertExclusiveRunnerProfile(Environment environment) {
		...
		if (mockActive && dockerActive) {
			throw new IllegalStateException(
				"Runner profile conflict: " + MOCK_PROFILE + " and " + DOCKER_PROFILE
