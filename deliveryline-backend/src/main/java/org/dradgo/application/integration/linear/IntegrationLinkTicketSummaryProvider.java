...
/**
 * Production {@link TicketSummaryProvider} that resolves the ticket summary projection from {@code
 * integration_links} (story 1.14 Task 5). Reads the active integration link's {@link
 * TicketSummaryProjection} through the application-owned SPI — no direct dependency on the
 * persistence adapter or JPA repositories, keeping the layered-boundary ArchUnit rules clean.
 *
 ...
 * integration_link exists for the run, falls back to a deterministic placeholder so tests that have
 * not yet wired a real link don't break — story 1.15 closes that gap by always creating an
...
public class IntegrationLinkTicketSummaryProvider implements TicketSummaryProvider {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(IntegrationLinkTicketSummaryProvider.class);
  ...
  public TicketSummary fetchByWorkflowRun(String workflowRunPublicId) {
    ...
    if (projection.isEmpty()) {
      log.debug(
          "ticket_summary fallback workflowRunId={} reason=no_active_integration_link",
          workflowRunPublicId);
    ...
    String summary =
        stringOrFallback(
            metadata, "summary", "Ticket summary unavailable on the integration_links row.");
  ...
  private static String stringOrFallback(
      Map<String, Object> metadata, String key, String fallback) {
