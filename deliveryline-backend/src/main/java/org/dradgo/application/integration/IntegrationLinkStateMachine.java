...
/**
 * Pure-Java state-machine guard for {@code integration_links.sync_status}. Used by the
 * persistence adapter to reject illegal transitions before they hit the DB, mirroring the
 * {@code RunnerExecutionStateMachine} pattern from story 1.13.
 *
 ...
 * </pre>
 *
 * <p>Reuses {@link DomainErrorCode#ILLEGAL_TRANSITION} — the registry already covers this
 * shape; the rejection details disambiguate the integration-link caller from the runner-execution
 * caller.
 */
public final class IntegrationLinkStateMachine {
	...
	private static final Set<IntegrationSyncStatus> TERMINAL = EnumSet.of(IntegrationSyncStatus.SUPERSEDED);

	static {
		Map<IntegrationSyncStatus, Set<IntegrationSyncStatus>> rules = new EnumMap<>(IntegrationSyncStatus.class);
		rules.put(IntegrationSyncStatus.LINKED, EnumSet.of(
			IntegrationSyncStatus.SYNCED,
			IntegrationSyncStatus.STALE,
			IntegrationSyncStatus.FAILED));
		rules.put(IntegrationSyncStatus.SYNCED, EnumSet.of(
			IntegrationSyncStatus.STALE,
			IntegrationSyncStatus.FAILED,
			IntegrationSyncStatus.SUPERSEDED));
		rules.put(IntegrationSyncStatus.STALE, EnumSet.of(
			IntegrationSyncStatus.SYNCED,
			IntegrationSyncStatus.FAILED,
			IntegrationSyncStatus.SUPERSEDED));
		rules.put(IntegrationSyncStatus.FAILED, EnumSet.of(
	...
	public static void assertCanTransition(
		...
		if (!targets.contains(to)) {
			...
			throw new DomainException(
				...
				"Illegal integration-link transition " + from.value() + " → " + to.value()
					+ " for " + integrationLinkPublicId,
				details);
		}
	}
}
