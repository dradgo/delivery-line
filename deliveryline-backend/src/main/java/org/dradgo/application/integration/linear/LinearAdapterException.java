 ...
 * is responsible for converting this to the appropriate {@code DomainException} or
 * {@code sync_status} transition (story 1.14 Task 5 / Task 8 logging contract).
 *
 * <p>This type is intentionally domain-shaped — it carries an {@link IntegrationFailureCategory}
 * rather than HTTP status codes or GraphQL error envelopes. The
 * {@code LINEAR_TYPES_MUST_NOT_LEAK_THROUGH_PORT} ArchUnit rule (Task 7) forbids GraphQL-specific
 * types in the {@code application.integration.linear} package; this exception remains compliant.
 */
public final class LinearAdapterException extends RuntimeException {
	...
	public LinearAdapterException(IntegrationFailureCategory failureCategory, String message, Throwable cause) {
		super(message, cause);
