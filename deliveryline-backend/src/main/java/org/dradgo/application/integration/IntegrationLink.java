...
/**
 * Domain shape of an {@code integration_links} row. The wire form of {@code integrationType} is the
 * V1 CHECK-constrained value ({@code linear} or {@code github_pr}); {@code externalRef} is the
 * provider-side ticket reference (e.g., {@code LIN-123}).
 *
 * <p>The persistence adapter translates entity ↔ this record via an explicit mapper (see story Task
 * 2). {@code archivedAt} is exposed so the service layer can distinguish active vs superseded links
...
public record IntegrationLink(
  ...
  /**
   * An "active" integration link is one that has not been archived and is not in a superseded state
   * — used by the conflict-detection path in {@code IntegrationLinkService.linkTicket}.
   */
