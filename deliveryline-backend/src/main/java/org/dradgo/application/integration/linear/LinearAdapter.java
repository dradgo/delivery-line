 ...
 * <ul>
 *   <li>{@code LinearMockAdapter} ({@code @Profile("linear-mock")}) — deterministic, fixture-backed,
 *       zero network calls. Default in {@code test}; opt-in for {@code local} / {@code demo}.</li>
 *   <li>{@code LinearRealAdapter} ({@code @Profile("linear-real")}) — Linear GraphQL polling +
 *       comment-posting. Opt-in only; never in any default profile group.</li>
 * </ul>
 *
 * <p>Only {@code IntegrationLinkService} and the polling host bean may call this port directly —
 * CLI / REST / persistence must go through {@code IntegrationLinkService} (mirrors the "only
 * {@code RunnerBroker} may call {@code RunnerAdapter.dispatch}" pattern from story 1.13).
 */
public interface LinearAdapter {

	/**
	 * Look up a Linear ticket by its external reference (e.g., {@code "LIN-123"}). Returns empty
	 * when the ticket does not exist at the source — the adapter does not throw on
	 * "not found". The command layer translates an empty result to a typed
	 * {@code LINEAR_TICKET_NOT_FOUND} domain exception (story 1.14 AC7).
	 *
	...
	/**
	 * Best-effort write-back of a governed-run summary to the source ticket. Idempotent on
	 * {@code (ticketRef, summary.runPublicId(), summary.fingerprint())} — re-posting the same
	 * fingerprint is a no-op (AC3 idempotency requirement). The {@code summary.body()} is expected
	 * to have already been through the redaction policy; the adapter does not redact.
	 */
