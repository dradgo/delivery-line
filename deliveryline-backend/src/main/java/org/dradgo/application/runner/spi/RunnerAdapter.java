...
public interface RunnerAdapter {
   ...
   * Docker container id, etc.) for later correlation. Implementations must be idempotent on {@code
   * request.runnerExecutionId()}: re-dispatch with the same id either no-ops or returns the prior
   * ack.
   */
