 ...
 *
 * <p>The class lives in {@code application.runner} (not {@code infrastructure}) so the
 * application services that consume it satisfy the architecture rule "application code must
 * not depend directly on infrastructure classes". The Spring wiring that activates binding
 * still lives in {@code infrastructure.config.RunnerConfiguration} via
