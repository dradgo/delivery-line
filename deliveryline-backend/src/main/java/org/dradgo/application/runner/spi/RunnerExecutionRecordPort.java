...
/**
 * Application-owned persistence boundary for {@code runner_executions} rows. Implementations
 * live under {@code adapters.persistence}. State-transition rules (pending → running →
...
public interface RunnerExecutionRecordPort {
	...
	/**
	 * Find pending/running rows whose {@code timeout_at < now()} server-side (no JVM clock).
	 * Bounded by {@code limit}.
	 */
	 ...
	 * inserted with {@code status = pending}, {@code last_activity_at = now()},
	 * {@code timeout_at = now() + executionConstraints.timeout}, and the supplied bundle version.
	 */
	...
	RunnerExecutionSnapshot touchActivity(String publicId, OffsetDateTime lastActivityAt, Duration staleTimeoutWindow);
