...
/**
 * Application-owned port for the runner-scratch directory under {@code deliveryline.home}.
 * The local filesystem implementation lives in {@code adapters.files} so neither the
 * application layer nor sibling adapters depend on filesystem types directly.
 */
public interface RunnerScratchStore {
	 ...
	 * field. The reference is resolved as a relative multi-segment path under
	 * {@code {deliveryline.home}/runner-scratch/{runnerExecutionId}/}, with the same
	 * containment guard as {@link #tryReadRunnerResult}: absolute paths, parent-segment escapes,
	 * and symlink swaps that would resolve outside {@code deliveryline.home} are rejected and
	 * surface as an empty {@link Optional}.
	 */
	 ...
	 */
	Path writeArtifactContent(String runnerExecutionId, String relativeReference, byte[] payloadBytes);
}
