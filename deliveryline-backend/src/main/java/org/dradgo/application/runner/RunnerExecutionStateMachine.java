 ...
 * shape (no new code is needed; the rejection details disambiguate the runner vs workflow
 * caller).
 */
public final class RunnerExecutionStateMachine {
	...
	private static final Set<RunnerExecutionStatus> TERMINAL = EnumSet.of(
	...
	static {
		Map<RunnerExecutionStatus, Set<RunnerExecutionStatus>> rules = new EnumMap<>(RunnerExecutionStatus.class);
		rules.put(RunnerExecutionStatus.PENDING, EnumSet.of(
			RunnerExecutionStatus.RUNNING,
			RunnerExecutionStatus.COMPLETED,
			RunnerExecutionStatus.FAILED,
			RunnerExecutionStatus.TIMED_OUT,
			RunnerExecutionStatus.ORPHANED));
		rules.put(RunnerExecutionStatus.RUNNING, EnumSet.of(
			RunnerExecutionStatus.COMPLETED,
			RunnerExecutionStatus.FAILED,
			RunnerExecutionStatus.TIMED_OUT,
			RunnerExecutionStatus.ORPHANED));
	...
	public static void assertCanTransition(
		...
		RunnerExecutionStatus from,
		RunnerExecutionStatus to
	) {
		...
		if (!targets.contains(to)) {
			...
			throw new DomainException(
				...
				"Illegal runner-execution transition " + from.value() + " → " + to.value()
					+ " for " + runnerExecutionId,
				details);
		}
	}
}
