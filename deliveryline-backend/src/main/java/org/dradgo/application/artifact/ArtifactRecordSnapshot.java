...
public record ArtifactRecordSnapshot(
	 ...
	 *
	 * <p><strong>Footgun:</strong> when used on a real read path it silently discards
	 * persisted failure metadata. The persistence-mapper read path
	 * ({@link org.dradgo.adapters.persistence.mapper.ArtifactEntityMapper}) must keep
	 * calling the canonical 13-arg constructor.
	 *
	 * <p>Prefer {@link #withoutFailureMetadata} in test code so the omission of failure
	 * metadata is named and intentional.
	 *
	 * @deprecated Will be removed once all in-tree callers migrate to
	 *             {@link #withoutFailureMetadata}; new code must not use this constructor.
	 */
