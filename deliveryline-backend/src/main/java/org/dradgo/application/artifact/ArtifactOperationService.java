...
public class ArtifactOperationService {
  ...
  private static final EnumSet<WorkflowState> TERMINAL_RUN_STATES =
  ...
  public ArtifactRecordSnapshot createDraft(
    ...
    ArtifactDraftRequest request =
        new ArtifactDraftRequest(
            ...
            null);
    log.info(
        "createDraft start workflowRunId={} artifactType={} actorIdentity={} actorType={} correlationId={}",
        workflowRunId,
        artifactType.value(),
        actor.actorIdentity(),
        actor.actorType(),
    ...
    log.info(
        "createDraft success workflowRunId={} artifactType={} artifactId={} version={}",
        workflowRunId,
        artifactType.value(),
        artifact.publicId(),
        artifact.version());
  ...
  public RecordArtifactOperationResult recordOperation(RecordArtifactOperationCommand operation) {
    ...
    try {
      ActorContext actor =
          new ActorContext(
              operation.actorIdentity(), operation.actorType(), operation.correlationId());
      String operationTypeValue = operation.operationType().value();
      log.info(
          "recordOperation start workflowRunId={} artifactType={} operationType={} idempotencyKey={} actorIdentity={} correlationId={} runnerExecutionId={}",
      ...
      Optional<RecordArtifactOperationResult> replayed =
          replayIfPresent(operation, operationTypeValue);
  ...
  public ArtifactAvailabilityResult markAvailable(
      String artifactId, ArtifactChecksum checksum, String storageRef, ActorContext actor) {
    requireActor(actor);
    ...
    try {
      log.info(
          "markAvailable start artifactId={} storageRef={} checksumAlgorithm={} actorIdentity={} correlationId={}",
          artifactId,
          storageRef,
          checksum.algorithm(),
          actor.actorIdentity(),
      ...
      Optional<ArtifactOperationSnapshot> pendingOp =
          artifactOperationPort.findPendingByArtifactId(artifactId);
      if (pendingOp.isEmpty()) {
        // D9: hybrid retry — look up current artifact state instead of throwing immediately.
        ArtifactRecordSnapshot currentArtifact =
            artifactRecordPort
        ...
        if (currentArtifact.status() == ArtifactStatus.AVAILABLE) {
          log.info(
              "markAvailable idempotent: artifact already AVAILABLE artifactId={}", artifactId);
          idempotencyReplayCounter.increment();
          return new ArtifactAvailabilityResult(currentArtifact, null);
        }
        log.warn(
            "markAvailable rejected: no pending operation and artifact not AVAILABLE artifactId={} artifactStatus={}",
            artifactId,
        ...
        throw new DomainException(
            ...
            "No pending operation found for artifact and artifact is not in an available state: "
                + currentArtifact.status().value(),
            Map.of("artifactId", artifactId, "artifactStatus", currentArtifact.status().value()));
      }
      ...
      try {
        ...
        if (stateError.errorCode() == DomainErrorCode.ARTIFACT_INVALID_STATE_TRANSITION) {
          ...
          log.warn(
              "markAvailable rejected: invalid artifact state transition, closing pending operation artifactId={} operationId={}",
              artifactId,
          ...
          try {
            ...
            log.error(
                "markAvailable: failed to close orphaned operation row after state transition error artifactId={} operationId={} closeError={}",
                artifactId,
                pendingOp.get().publicId(),
                closeError.toString());
          }
      ...
      if (artifact.status() != ArtifactStatus.AVAILABLE) {
        throw new DomainException(
            ...
            "markAvailable did not transition artifact to AVAILABLE state: "
                + artifact.status().value(),
            Map.of("artifactId", artifactId, "unexpectedStatus", artifact.status().value()));
      }

      ArtifactOperationSnapshot operation =
          artifactOperationPort.markComplete(pendingOp.get().publicId());
  ...
  public ArtifactFailureResult markFailed(
      String artifactId, FailureCategory failureCategory, String reason, ActorContext actor) {
    requireActor(actor);
    validateFailureReason(reason);
    String priorArtifactMdc = MdcKeys.beginScope(MdcKeys.ARTIFACT_ID, artifactId);
    try {
      log.info(
          "markFailed start artifactId={} failureCategory={} actorIdentity={} correlationId={}",
          artifactId,
      ...
      if (pendingOp.isEmpty()) {
        ...
        log.warn(
            "markFailed rejected: no pending operation and artifact not FAILED artifactId={} artifactStatus={}",
            artifactId,
            currentArtifact.status().value());
        throw new DomainException(
            DomainErrorCode.ARTIFACT_OPERATION_INTENT_CONFLICT,
      ...
      ArtifactRecordSnapshot artifact =
          artifactRecordPort.markFailed(artifactId, failureCategory, reason);
      ArtifactOperationSnapshot operation =
          artifactOperationPort.markFailed(pendingOp.get().publicId(), failureCategory, reason);
      artifactEventPort.append(
          new ArtifactEventRecord(
              artifact.workflowRunId(),
      ...
      log.warn(
          "markFailed applied artifactId={} operationId={} workflowRunId={} failureCategory={} failureReason={}",
          artifact.publicId(),
          operation.publicId(),
          artifact.workflowRunId(),
          failureCategory.value(),
  ...
  private Optional<RecordArtifactOperationResult> replayIfPresent(
      RecordArtifactOperationCommand operation, String operationTypeValue) {
    Optional<ArtifactOperationSnapshot> replay =
        artifactOperationPort.findReplay(
            operation.workflowRunId(),
    ...
    ArtifactRecordSnapshot priorArtifact =
        artifactRecordPort
            .findByPublicId(priorOperation.artifactId())
            .orElseThrow(
                () -> {
                  log.warn(
                      "recordOperation replay rejected: artifact record missing for replayed operation operationId={} artifactId={}",
                      priorOperation.publicId(),
                      priorOperation.artifactId());
                  return new DomainException(
                      DomainErrorCode.ARTIFACT_RECORD_NOT_FOUND,
                      "Artifact record missing for replayed operation: "
                          + priorOperation.artifactId(),
                      Map.of(
  ...
  private RecordArtifactOperationResult finalizeReplay(
    ...
    if (result.isFailure()) {
      ...
      log.warn(
          "recordOperation replay returned failed prior operation workflowRunId={} idempotencyKey={} priorOperationId={} failureCategory={}",
          operation.workflowRunId(),
          operation.idempotencyKey(),
          failure.operation().publicId(),
          failure.operation().failureCategory() != null
              ? failure.operation().failureCategory().value()
              : "none");
      throw new DomainException(
          DomainErrorCode.ARTIFACT_OPERATION_INTENT_CONFLICT,
    ...
    log.warn(
        "recordOperation replay hit workflowRunId={} idempotencyKey={} operationType={} priorOperationId={} priorOperationStatus={} priorArtifactId={}",
        operation.workflowRunId(),
        operation.idempotencyKey(),
        operationTypeValue,
        result.operation().publicId(),
  ...
  private RecordArtifactOperationResult executeRecordOperation(
    ...
    return operationTemplate.execute(
        status -> doRecordOperationInTransaction(operation, operationTypeValue, actor));
  }
  ...
  private RecordArtifactOperationResult doRecordOperationInTransaction(
    ...
    artifactRecordPort.lockLineageForUpdate(
    ...
    try {
      ...
      try {
        if (operation.runnerExecutionId() != null
            && artifactRunnerExecutionPort.isTimedOut(operation.runnerExecutionId())) {
          log.warn(
              "recordOperation flagging artifact as late_or_stale workflowRunId={} artifactId={} runnerExecutionId={}",
              operation.workflowRunId(),
              artifact.publicId(),
              operation.runnerExecutionId());
        ...
        ArtifactOperationSnapshot pending =
            artifactOperationPort.createPending(
                operationPublicId,
                operation.workflowRunId(),
                operation.artifactType(),
                artifact.publicId(),
  ...
  private RecordArtifactOperationResult handleRecordOperationConstraintCollision(
    ...
    if (causeMessage == null || !causeMessage.contains(IDEMPOTENCY_CONSTRAINT)) {
      log.warn(
          "recordOperation unrelated constraint violation workflowRunId={} artifactType={} operationType={} idempotencyKey={} cause={}",
          operation.workflowRunId(),
          operation.artifactType().value(),
          operationTypeValue,
          operation.idempotencyKey(),
          duplicate.getMostSpecificCause().getClass().getSimpleName());
      throw new DomainException(
    ...
    Optional<RecordArtifactOperationResult> replayed =
        replayIfPresentAfterCollision(operation, operationTypeValue);
    if (replayed.isPresent()) {
      return finalizeReplay(operation, operationTypeValue, replayed.get());
    }

    log.warn(
        "recordOperation idempotency constraint collision without replay winner workflowRunId={} artifactType={} operationType={} idempotencyKey={} cause={}",
        operation.workflowRunId(),
        operation.artifactType().value(),
        operationTypeValue,
        operation.idempotencyKey(),
  ...
  private Optional<RecordArtifactOperationResult> replayIfPresentAfterCollision(
    ...
    RecordArtifactOperationResult replayed =
        raceReplayTemplate.execute(
    ...
    return switch (operation.operationType()) {
      case REPLACE, UPDATE -> {
        if (latestArtifact.isEmpty()) {
          ...
          log.warn(
              "recordOperation rejected: UPDATE/REPLACE against empty lineage workflowRunId={} artifactType={} operationType={} idempotencyKey={}",
              operation.workflowRunId(),
              operation.artifactType().value(),
              operationTypeValue,
              operation.idempotencyKey());
          throw new DomainException(
              ...
              "Cannot "
                  + operationTypeValue
                  + " a non-existent artifact lineage for workflowRunId="
                  + operation.workflowRunId()
                  + ", artifactType="
                  + operation.artifactType().value()
      ...
      case CREATE -> {
        if (latestArtifact.isPresent() && latestArtifact.get().status() != ArtifactStatus.FAILED) {
          ...
          log.warn(
              "recordOperation rejected: lineage already exists workflowRunId={} artifactType={} existingArtifactId={} existingStatus={} existingVersion={}",
              operation.workflowRunId(),
              operation.artifactType().value(),
              existing.publicId(),
        ...
        yield artifactRecordPort.createDraft(
            new ArtifactDraftRequest(
                operation.workflowRunId(),
                operation.artifactType(),
                operation.payloadRef(),
  ...
  private void requireWorkflowRunNonTerminal(String workflowRunId) {
    ...
    if (TERMINAL_RUN_STATES.contains(state)) {
      log.warn(
          "recordOperation rejected: workflow run is terminal workflowRunId={} workflowRunState={}",
          workflowRunId,
          state.value());
      throw new DomainException(
          ...
          "Workflow run is in a terminal state and cannot accept artifact operations: "
              + state.value(),
          Map.of("workflowRunId", workflowRunId, "workflowRunState", state.value()));
    }
  }
  ...
  private void validateStorageRef(String storageRef) {
    ...
    // P21: path shape guard — reject absolute paths, scheme prefixes, and path-traversal
    // components.
  ...
  private void verifyPayloadMatchesChecksum(
      String artifactId, String storageRef, ArtifactChecksum checksum) {
    // P15: disallowed or unrecognized algorithm → INVALID_COMMAND_PAYLOAD (not
    ...
    if (canonical == null || !ArtifactChecksum.ALLOWED_ALGORITHMS.contains(canonical)) {
      log.warn(
          "markAvailable rejected: disallowed checksum algorithm artifactId={} algorithm={}",
          artifactId,
          checksum.algorithm());
      throw new DomainException(
          DomainErrorCode.INVALID_COMMAND_PAYLOAD,
          ...
          Map.of("artifactId", artifactId, "checksumAlgorithm", checksum.algorithm()));
    }
    Optional<byte[]> bytes = artifactPayloadStore.readBytes(storageRef);
    if (bytes.isEmpty()) {
      log.warn(
          "markAvailable rejected: payload unreadable artifactId={} storageRef={}",
          artifactId,
          storageRef);
      throw new DomainException(
    ...
    if (payload.length == 0) {
      log.warn(
          "markAvailable rejected: payload is empty artifactId={} storageRef={}",
          artifactId,
          storageRef);
      throw new DomainException(
          DomainErrorCode.ARTIFACT_PAYLOAD_UNAVAILABLE,
    ...
    String recomputedHex =
        ArtifactChecksum.digestHex(checksum.algorithm(), payload)
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "digestHex returned empty for allowlisted algorithm " + canonical));
    ...
    try {
      ...
      log.warn(
          "markAvailable rejected: supplied checksum is not valid hex artifactId={} algorithm={}",
          artifactId,
          checksum.algorithm());
      throw new DomainException(
    ...
    if (!MessageDigest.isEqual(recomputedBytes, suppliedBytes)) {
      log.warn(
          "markAvailable rejected: checksum mismatch artifactId={} algorithm={} payloadLength={}",
          artifactId,
          checksum.algorithm(),
          payload.length);
      throw new DomainException(
  ...
  private void validateFailureReason(String reason) {
    // V2's ck_artifacts_failure_reason_paired enforces (failure_category IS NULL) = (failure_reason
    // IS NULL).
    // markFailed always sets failureCategory non-null, so a null/blank reason violates the
    // constraint
    // at flush time and crashes the request as a raw DataIntegrityViolationException. Reject up
    // front.
