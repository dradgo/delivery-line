...
public class ArtifactService {
	...
	public boolean isApprovalEligible(String artifactId) {
		Optional<ArtifactRecordSnapshot> maybeArtifact = artifactRecordPort.findByPublicId(artifactId)
			...
			.filter(artifact -> artifact.checksumValue() != null && !artifact.checksumValue().isBlank())
		...
		if (bytes.isEmpty()) {
			log.warn("isApprovalEligible=false artifactId={} storageRef={} reason=payloadUnreadable",
				artifactId, artifact.storageRef());
			return false;
		}
		byte[] payload = bytes.get();
		if (payload.length == 0) {
			log.warn("isApprovalEligible=false artifactId={} storageRef={} reason=emptyPayload",
				artifactId, artifact.storageRef());
			return false;
		}
		...
		if (!match) {
			log.warn("isApprovalEligible=false artifactId={} checksumAlgorithm={} reason=checksumMismatch payloadLength={}",
				artifactId, artifact.checksumAlgorithm(), payload.length);
		} else {
			log.info("isApprovalEligible=true artifactId={} checksumAlgorithm={} payloadLength={}",
				artifactId, artifact.checksumAlgorithm(), payload.length);
		}
		return match;
	}
