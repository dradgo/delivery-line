...
public interface IdempotencyRecordPort {
	...
	/**
	 * Attempt to insert a new RESERVED idempotency record. Race-safe via a single insert with conflict-ignore semantics.
	 *
	 * <p>Caller must be in an active transaction; implementations participate via REQUIRED propagation.
	 *
	 * @return {@code true} when a row was inserted by this call; {@code false} when an existing row already held the key
	 */
	boolean tryReserve(
		String publicId,
		String key,
	...
	/**
	 * Transition a RESERVED record to its terminal status (COMPLETED or FAILED) with the resolved result reference.
	 */
	void markCompleted(String key, String resultRef, IdempotencyRecordStatus status, OffsetDateTime completedAt);
}
