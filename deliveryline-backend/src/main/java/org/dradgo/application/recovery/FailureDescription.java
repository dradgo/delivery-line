 ...
 *
 * <p>Returned by {@link RecoveryService#describeFailure(String)} and consumed by
 * {@link org.dradgo.application.workflow.WorkflowInspectionService} to populate the CLI
 * {@code status} and REST inspection surfaces. All five {@code failed*} / {@code last*Activity*}
 * fields are non-null only when {@code currentState == FAILED}; on non-Failed runs they are all
 * null and {@code nextSafeAction} reflects the canonical non-Failed value
 * (e.g. {@code await_outcome}, {@code view_only}).
 *
 ...
 * @param nextSafeAction      one of {@code retry, await_manual_reconciliation, await_outcome,
 *                            view_only} (string literal — see the helper table in story Dev Notes)
 * @param diagnosticReferenceCorrelationId the most recent {@code correlationId} recorded in
 *                            {@code workflow_events.details} for this run, or null if none
 */
