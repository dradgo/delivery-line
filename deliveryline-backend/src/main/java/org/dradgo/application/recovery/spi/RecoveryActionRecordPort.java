 ...
 * (story 1.18 Task 3 — landing in a follow-up slice) can persist recovery rows without touching the
...
public interface RecoveryActionRecordPort {
   ...
   * resolves the workflow_run / workflow_events FK references by public-id lookup, and translates a
   * unique-violation on {@code uq_recovery_actions_idempotency_key} into {@code
   * DomainException(IDEMPOTENCY_KEY_CONFLICT)}.
   *
   * @throws org.dradgo.domain.DomainException with {@code IDEMPOTENCY_KEY_CONFLICT} if the
   *     idempotency_key collides with an existing row, or {@code INTERNAL_ERROR} if a referenced
   *     workflow_run / workflow_event public id is missing.
   */
  RecoveryActionSnapshot insert(RecoveryActionWriteCommand command);

  /**
   * Look up an existing recovery action by idempotency_key. Returns {@code Optional.empty()} if no
   * row matches — the recovery service uses this to detect replays before reserving the downstream
   * broker key.
   */
  ...
  /**
   * Flip {@code result_status} from {@code pending} to {@code failed}. Throws {@code
   * DomainException(INTERNAL_ERROR)} if no matching row exists or if the current {@code
   * result_status} is not {@code pending}.
   */
