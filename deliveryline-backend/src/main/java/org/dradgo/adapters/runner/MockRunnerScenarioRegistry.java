 ...
 *
 * <p>Profile-activated by {@code runners.mock} so the bean only loads when the mock runner stack is
 * wired.
 */
@Component
...
public class MockRunnerScenarioRegistry {
  ...
  public MockRunnerScenarioRegistry() {
    ...
    registerDefault(
        ...
        MockRunnerScenario.Behaviour.HAPPY,
        "happy-implementation-plan.json",
        null);
    registerDefault(
        "happy-pr-output",
        RunnerStage.EXECUTION,
    ...
    registerDefault(
        "timeout",
        null,
        MockRunnerScenario.Behaviour.TIMEOUT,
        null,
        FailureCategory.RUNNER_TIMEOUT);
    registerDefault(
        "crash", null, MockRunnerScenario.Behaviour.CRASH, null, FailureCategory.RUNNER_CRASH);
    registerDefault(
        "contract-violation",
        null,
        MockRunnerScenario.Behaviour.CONTRACT_VIOLATION,
        "contract-violation.json",
        FailureCategory.RUNNER_CONTRACT_VIOLATION);
    registerDefault(
        "non-zero-exit",
        null,
        MockRunnerScenario.Behaviour.NON_ZERO_EXIT,
        "non-zero-exit.json",
        FailureCategory.RUNNER_NON_ZERO_EXIT);
    registerDefault(
        "late-result",
        null,
        MockRunnerScenario.Behaviour.LATE_RESULT,
        "late-result.json",
        FailureCategory.RUNNER_LATE_RESULT);
    registerDefault(
        "duplicate-result",
        null,
        MockRunnerScenario.Behaviour.DUPLICATE_RESULT,
        "duplicate-result.json",
        FailureCategory.RUNNER_DUPLICATE_RESULT);
    registerDefault(
        "malformed-output",
        null,
        MockRunnerScenario.Behaviour.MALFORMED_OUTPUT,
  ...
  public byte[] loadFixtureBytes(MockRunnerScenario scenario) {
    ...
    try (InputStream stream =
      ...
      if (stream == null) {
        log.warn(
            "loadFixtureBytes missing scenario={} resource={}",
            scenario.name(),
