 ...
 *
 * <p>Active only under the {@code runners.mock} Spring profile. The Epic 3 {@code DockerRunnerAdapter}
 * activates under a future {@code runners.docker} profile (kept undefined in Epic 1).
 */
@Component
...
public class MockRunnerAdapter implements RunnerAdapter {
	...
	public RunnerDispatchAck dispatch(RunnerDispatchRequest request) {
		...
		String scenarioName = overrideName != null
			? overrideName
			: runnerProperties.mock().scenarioFor(request.stage());
		...
		log.info("mock dispatched runnerExecutionId={} workflowRunId={} stage={} scenario={} behaviour={}",
			request.runnerExecutionId(), workflowRunId, request.stage().value(),
			scenario.name(), scenario.behaviour());
	 ...
	 * (LATE_RESULT) and the "second result for the same runnerExecutionId" branch
	 * (DUPLICATE_RESULT) without depending on wall-clock or thread scheduling.
	 */
	public boolean releaseDeferredResult(String runnerExecutionId) {
		...
		log.info("mock released deferred result runnerExecutionId={} bytes={}", runnerExecutionId, payload.length);
		return true;
	}
	...
	private void executeScenarioSideEffect(RunnerDispatchRequest request, MockRunnerScenario scenario) {
		switch (scenario.behaviour()) {
			...
			case MALFORMED_OUTPUT -> {
				byte[] fixture = scenarioRegistry.loadFixtureBytes(scenario);
				byte[] tailored = fixture.length > 0
