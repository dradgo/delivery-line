...
/**
 * Adapts {@link RunnerExecutionEventPort} onto the canonical
 * {@link WorkflowEventWritePort}. The port restricts the type whitelist to
 * {@link WorkflowEventType#RUNNER_STARTED},
 * {@link WorkflowEventType#RUNNER_FAILED}, and
 * {@link WorkflowEventType#RECOVERY_RECONCILED} so callers cannot accidentally append
 * {@code WORKFLOW_STATE_CHANGED} (which must go through {@code WorkflowTransitionService}).
 */
@Component
public class RunnerExecutionEventPersistenceAdapter implements RunnerExecutionEventPort {

	private static final java.util.Set<WorkflowEventType> ALLOWED_EVENT_TYPES = java.util.EnumSet.of(
	...
	public void append(
		...
		if (!ALLOWED_EVENT_TYPES.contains(eventType)) {
			throw new IllegalArgumentException(
				"RunnerExecutionEventPort only accepts " + ALLOWED_EVENT_TYPES
					+ " but received " + eventType.value());
		}
