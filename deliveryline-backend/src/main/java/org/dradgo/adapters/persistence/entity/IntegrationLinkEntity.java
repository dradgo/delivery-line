 ...
 * constraints; the persistence adapter is responsible for never inserting a value that violates a
 * CHECK (state machine guards {@code sync_status}; the SPI's {@code byte[] externalMetadata} has
 * its 64KB ceiling validated at the application boundary before reaching this entity).
 *
 * <p>{@code externalMetadata} is mapped as a Hibernate-managed {@code Map} over a {@code jsonb}
 * column (matches the {@code workflow_events.details} pattern); the persistence adapter translates
 * raw {@code byte[]} from the SPI to/from the map via Jackson.
 */
@Entity
