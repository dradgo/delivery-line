...
public class ArtifactOperationPersistenceAdapter implements ArtifactOperationPort {
	...
	private static final Logger log = LoggerFactory.getLogger(ArtifactOperationPersistenceAdapter.class);
	...
	public Optional<ArtifactOperationSnapshot> findReplay(
		...
		return artifactOperationRepository.findFirstByWorkflowRunPublicIdAndArtifactTypeAndIdempotencyKeyAndOperationType(
			workflowRunId,
			artifactType.value(),
			idempotencyKey,
			operationType).map(artifactOperationEntityMapper::toSnapshot);
	}

	@Override
	@Transactional(readOnly = true)
	public Optional<ArtifactOperationSnapshot> findPendingByArtifactId(String artifactId) {
		List<ArtifactOperationEntity> pending = artifactOperationRepository.findByArtifactPublicIdAndStatusOrderByCreatedAtDesc(
			artifactId,
		...
		if (pending.size() > 1) {
			...
			log.error("findPendingByArtifactId invariant violated: multiple pending operations exist artifactId={} pendingOperationIds={}",
				artifactId, publicIds);
	...
	public ArtifactOperationSnapshot createPending(
		...
		ArtifactOperationSnapshot persisted = artifactOperationEntityMapper.toSnapshot(artifactOperationRepository.saveAndFlush(entity));
		log.info("artifact_operation pending persisted operationId={} workflowRunId={} artifactId={} artifactType={} operationType={} idempotencyKey={}",
			persisted.publicId(), workflowRunId, artifactId, artifactType.value(), operationType, idempotencyKey);
		return persisted;
	}
	...
	public List<ArtifactOperationSnapshot> findPendingOlderThan(Duration threshold) {
		...
		return artifactOperationRepository.findByStatusAndCreatedAtOlderThanSeconds(
			ArtifactOperationStatus.PENDING.value(),
			seconds).stream()
	...
	public ArtifactOperationSnapshot markFailedOrphan(String operationPublicId, String reason) {
		...
		operation.setReason(reason);
		ArtifactOperationSnapshot persisted = artifactOperationEntityMapper.toSnapshot(artifactOperationRepository.saveAndFlush(operation));
		log.warn("artifact_operation marked failed_orphan operationId={} artifactId={} reason={}",
			persisted.publicId(), persisted.artifactId(), reason);
		return persisted;
	}
	...
	private ArtifactOperationEntity requireOperation(String operationPublicId) {
		return artifactOperationRepository.findByPublicId(operationPublicId)
			.orElseThrow(() -> new DomainException(
				DomainErrorCode.ARTIFACT_RECORD_NOT_FOUND,
