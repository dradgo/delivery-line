 ...
 * with V1 / V6 constraint awareness — unique violations on {@code integration_links} are
 * translated to {@code INTEGRATION_LINK_CONFLICT} regardless of whether they originate from the
 * V1 (same ticket, same run) or V6 partial (same ticket, any active run) index.
 *
...
public class IntegrationLinkPersistenceAdapter implements IntegrationLinkRecordPort {
	...
	private static final Logger log = LoggerFactory.getLogger(IntegrationLinkPersistenceAdapter.class);
	...
	IntegrationLinkPersistenceAdapter(
		...
		this.integrationLinkRepository = Objects.requireNonNull(integrationLinkRepository, "integrationLinkRepository");
	...
	public IntegrationLink insert(NewIntegrationLink request) {
		...
		byte[] metadataBytes = request.externalMetadata() == null ? new byte[0] : request.externalMetadata();
		...
		WorkflowRunEntity workflowRun = workflowRunRepository.findByPublicId(request.workflowRunPublicId())
		...
		entity.setLastSyncAt(request.lastSyncAt() == null
			? null
	...
	@Override
	public Optional<IntegrationLink> findActiveByTypeAndExternalRef(String integrationType, String externalRef) {
	...
	@Override
	public Optional<TicketSummaryProjection> findActiveTicketSummaryByWorkflowRun(String workflowRunPublicId) {
		PublicIdPrefixes.require(workflowRunPublicId, PublicIdPrefixes.WORKFLOW_RUN);
		return integrationLinkRepository
			.findFirstActiveByWorkflowRunPublicId(workflowRunPublicId)
			.map(entity -> new TicketSummaryProjection(
	...
	private byte[] encodeMetadata(Map<String, Object> metadata) {
		...
		try {
			...
			log.warn("findActiveTicketSummaryByWorkflowRun metadata_serialize_failed workflowRunId={}",
	...
	public IntegrationLink updateSyncStatus(
		...
		IntegrationLinkEntity entity = integrationLinkRepository.findByPublicId(publicId)
	...
	public IntegrationLink markArchived(String publicId, Instant archivedAt) {
		...
		IntegrationLinkEntity persisted = integrationLinkRepository.saveAndFlush(entity);
		log.info("integration_link archived publicId={} archivedAt={}", publicId, persisted.getArchivedAt());
		return mapper.toDomain(persisted);
	}
	...
	private static DomainException conflict(NewIntegrationLink request, DataIntegrityViolationException cause) {
		Map<String, Object> details = new LinkedHashMap<>();
		details.put("integrationLinkId", request.publicId());
		...
		String message = cause.getMostSpecificCause() == null
			? null
	...
	private static DomainException integrationLinkNotFound(String publicId) {
		...
		// an INTEGRATION_LINK_NOT_FOUND code (the externally-facing not-found is LINEAR_TICKET_NOT_FOUND
		...
		return new DomainException(
			...
			"Integration link not found: " + publicId,
			details);
	}
