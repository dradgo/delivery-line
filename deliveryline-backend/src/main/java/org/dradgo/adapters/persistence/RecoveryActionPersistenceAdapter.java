 ...
 *
 * <p>Translates unique-violations on {@code uq_recovery_actions_idempotency_key} into
 * {@code DomainException(IDEMPOTENCY_KEY_CONFLICT)} mirroring the pattern established by
 * {@link IntegrationLinkPersistenceAdapter#insert(IntegrationLinkPersistenceAdapter.NewIntegrationLink)}.
 *
...
public class RecoveryActionPersistenceAdapter implements RecoveryActionRecordPort {
	...
	public RecoveryActionPersistenceAdapter(
		...
		this.recoveryActionRepository = Objects.requireNonNull(recoveryActionRepository, "recoveryActionRepository");
		this.workflowRunRepository = Objects.requireNonNull(workflowRunRepository, "workflowRunRepository");
		this.workflowEventRepository = Objects.requireNonNull(workflowEventRepository, "workflowEventRepository");
	...
	public RecoveryActionSnapshot insert(RecoveryActionWriteCommand command) {
		...
		WorkflowRunEntity workflowRun = workflowRunRepository.findByPublicId(command.workflowRunPublicId())
			.orElseThrow(() -> referenceMissing("workflowRunPublicId", command.workflowRunPublicId()));

		WorkflowEventEntity triggeringEvent = resolveOptionalEvent(command.triggeringEventPublicId(), "triggeringEventPublicId");
		WorkflowEventEntity resultingEvent = resolveOptionalEvent(command.resultingEventPublicId(), "resultingEventPublicId");
		...
		try {
			...
			log.warn(
				...
				collision.getMostSpecificCause() == null ? null : collision.getMostSpecificCause().getClass().getSimpleName());
	...
	private RecoveryActionSnapshot flipTerminal(String idempotencyKey, String targetStatus) {
		requireNonBlank("idempotencyKey", idempotencyKey);
		RecoveryActionEntity entity = recoveryActionRepository.findByIdempotencyKey(idempotencyKey)
			.orElseThrow(() -> {
		...
		if (!STATUS_PENDING.equals(current)) {
			...
			throw new DomainException(
				DomainErrorCode.INTERNAL_ERROR,
				"recovery_actions.result_status is already terminal (" + current + "); cannot transition to " + targetStatus,
				details);
		}
	...
	private WorkflowEventEntity resolveOptionalEvent(String publicId, String fieldName) {
		...
		return workflowEventRepository.findByPublicId(publicId)
	...
	private static DomainException translateUniqueViolation(
		...
		String message = cause.getMostSpecificCause() == null ? null : cause.getMostSpecificCause().getMessage();
		if (message != null) {
		...
		return new DomainException(
			DomainErrorCode.INTERNAL_ERROR,
			"recovery_actions constraint violation",
			details);
	}
