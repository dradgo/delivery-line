...
public class WorkflowCliExitStatusExceptionMapper implements ExitStatusExceptionMapper {
	...
	private int exitCodeFor(DomainErrorCode errorCode) {
		return switch (errorCode) {
			case IDEMPOTENCY_KEY_CONFLICT, STALE_IDEMPOTENCY_RESERVATION, CONCURRENT_TRANSITION_CONFLICT,
					APPROVAL_VERSION_MISMATCH, EXPORT_CLASSIFICATION_VIOLATION, IDEMPOTENCY_RESERVATION_EXHAUSTED,
					RETRY_NOT_APPLICABLE -> 201;
			case MISSING_IDEMPOTENCY_KEY, INVALID_IDEMPOTENCY_KEY, INVALID_TIME_RANGE, HISTORY_TOO_LARGE -> 101;
