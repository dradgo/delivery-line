...
public class WorkflowCliExitStatusExceptionMapper implements ExitStatusExceptionMapper {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(WorkflowCliExitStatusExceptionMapper.class);
  ...
  public ExitStatus apply(Exception exception) {
    if (exception instanceof DomainException domainException) {
      return new ExitStatus(
          exitCodeFor(domainException.errorCode()),
          format(domainException.errorCode(), domainException.getMessage()));
    }
    LOG.error("Unexpected exception escaped CLI command; surfacing INTERNAL_ERROR", exception);
    return new ExitStatus(
        401, format(DomainErrorCode.INTERNAL_ERROR, GENERIC_INTERNAL_ERROR_DETAIL));
  }

  private int exitCodeFor(DomainErrorCode errorCode) {
    return switch (errorCode) {
      case IDEMPOTENCY_KEY_CONFLICT,
          STALE_IDEMPOTENCY_RESERVATION,
      ...
      case DOCTOR_POSTGRES_UNREACHABLE,
          ...
          DOCTOR_DOCKER_MISSING,
          DOCTOR_CONFIG_PERMISSIONS_UNSAFE,
          DOCTOR_UNSUPPORTED_ENVIRONMENT,
