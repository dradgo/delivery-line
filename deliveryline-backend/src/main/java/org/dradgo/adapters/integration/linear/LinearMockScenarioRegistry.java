...
/**
 * In-memory deterministic registry of {@link LinearMockScenario} entries keyed by
 * {@code ticketRef}. Production-classpath fixtures live under
 * {@code src/main/resources/linear-fixtures/}; tests may register adversarial scenarios via
 * {@link #register(LinearMockScenario)} (cleanup with {@link #clearTestScenarios()}).
 *
 * <p>Profile-activated by {@code linear-mock} so the bean only loads when the mock Linear stack
 * is wired (mirrors {@code MockRunnerScenarioRegistry}).
 *
...
public class LinearMockScenarioRegistry {
	...
	public LinearMockScenarioRegistry() {
		registerDefault(TICKET_FEATURE_LOW_RISK, LinearMockScenario.Behaviour.HAPPY,
			"linear-feature-low-risk.json", null);
		registerDefault(TICKET_BUG_FIX, LinearMockScenario.Behaviour.HAPPY,
			"linear-bug-fix.json", null);
		registerDefault(TICKET_DOCS, LinearMockScenario.Behaviour.HAPPY,
			"linear-docs.json", null);
	}
	...
	public LinearTicket loadHappyFixture(LinearMockScenario scenario) {
		...
		if (scenario.behaviour() != LinearMockScenario.Behaviour.HAPPY) {
			throw new IllegalStateException(
				"loadHappyFixture only supports HAPPY scenarios (ticketRef=" + scenario.ticketRef() + ")");
		}
		...
		try (InputStream stream = Thread.currentThread().getContextClassLoader().getResourceAsStream(resource)) {
			...
			LinearTicketFixtureDocument document = objectMapper.readValue(stream, LinearTicketFixtureDocument.class);
