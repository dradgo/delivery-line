...
/**
 * Single deterministic behaviour for a Linear mock ticket. Keyed by {@code ticketRef} in {@link
 * LinearMockScenarioRegistry}. Mirrors the 1-13 {@code MockRunnerScenario} shape; the only
 * non-fixture-driven entries are {@link Behaviour#NOT_FOUND} (no fixture) and the adversarial
 * test-only branches ({@link Behaviour#RATE_LIMITED}, {@link Behaviour#NETWORK_FAILURE}, {@link
 * Behaviour#AUTH_FAILURE}, {@link Behaviour#MALFORMED_RESPONSE}) which simulate failure paths
 * without loading a fixture body.
 *
 * <p>{@code fixtureResource} is a classpath path (e.g., {@code
 * "linear-fixtures/linear-bug-fix.json"}) — production fixtures live under {@code
 * src/main/resources/linear-fixtures/} and adversarial-failure markers live under {@code
 * src/test/resources/linear-fixtures/} so the latter never leak into the {@code demo}/{@code local}
 * runtime classpath.
 *
