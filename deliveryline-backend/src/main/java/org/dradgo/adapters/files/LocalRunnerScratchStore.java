 ...
 *
 * <ul>
 *   <li>{@code context-bundle.v1.json} — written by {@code RunnerBroker.dispatch} after the DB row
 *       commits;
 *   <li>{@code runner-result.v1.json} — written by {@code MockRunnerAdapter} (Epic 1) or the real
 *       Docker runner (Epic 3), read back by the broker on result handling.
 * </ul>
 *
 * <p>Containment guards mirror {@link LocalArtifactStore}'s pattern: the {@code rex_} prefix is
 * validated through {@link PublicIdPrefixes}, the resolved path is asserted to stay under {@code
 * deliveryline.home}, and {@link LinkOption#NOFOLLOW_LINKS} blocks the symlink-swap race on the
 * read path.
 */
@Component
public class LocalRunnerScratchStore implements RunnerScratchStore {
  ...
  @Override
  public Path writeArtifactContent(
    ...
    Path target =
        resolveArtifactPath(runnerExecutionId, relativeReference)
            .orElseThrow(
                () -> {
    ...
    log.info(
        "scratch artifact written runnerExecutionId={} reference={} bytes={}",
        runnerExecutionId,
        relativeReference,
        payloadBytes.length);
    return target;
  ...
  private Path atomicWrite(String runnerExecutionId, String filename, byte[] payload) {
    ...
    log.info(
        "scratch file written runnerExecutionId={} filename={} bytes={}",
        runnerExecutionId,
        filename,
        payload.length);
    return target;
  ...
  private Path scratchFile(String runnerExecutionId, String filename) {
    ...
    Path target =
        deliverylineHome
            .resolve(SCRATCH_ROOT)
            .resolve(runnerExecutionId)
            .resolve(filename)
            .normalize();
