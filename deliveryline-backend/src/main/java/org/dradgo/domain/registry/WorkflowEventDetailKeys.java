 ...
 * surfaces — producer ({@code RecoveryService}, {@code WorkflowCommandService}), allow-list
 * ({@code WorkflowInspectionService}), JSON schema ({@code workflow-history.v1.schema.json}), and
 * the PostgreSQL-native query in {@code WorkflowEventRepository}.
 *
 ...
 * centralized them here and added a contract test that pins the holder against the JSON schema
 * and the allow-list. The SQL string literal in {@code WorkflowEventRepository} cannot reference
 * a Java constant inside an {@code @Query} annotation, so the contract test compares the JPQL
 * source text to {@link #CORRELATION_ID}.
 *
...
public final class WorkflowEventDetailKeys {
	...
	public static final List<String> ALLOW_LISTED_KEYS = List.of(
