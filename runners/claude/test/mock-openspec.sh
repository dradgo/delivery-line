...
# only when the Dockerfile is built with --build-arg INSTALL_CLAUDE_CLI=false (the
...
# Identical to runners/codex/test/mock-openspec.sh — per-runner copies mirror the
# existing mock-codex.sh / mock-claude.sh convention (RUNNER_CONTRACT mirror rule).
set -eu
