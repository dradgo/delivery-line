...
# prompt selection (claude). Twin of runners/codex/test/entrypoint-review-directive.test.sh.
#
...
# Run locally: `sh runners/claude/test/entrypoint-review-directive.test.sh`
set -eu
...
MARKER_SPEC="ADVISORY SPECIFICATION REVIEW TASK"
MARKER_PLAN="ADVISORY IMPLEMENTATION-PLAN REVIEW TASK"
MARKER_IMPL="ADVISORY REVIEW TASK"

NODE_BIN="${NODE_BIN:-node}"
FAILURES=0
WORKROOT="$(mktemp -d "${TMPDIR:-/tmp}/claude-ep-review-directive.XXXXXX")"
...
run_review() {
  ...
  _mock="$_w/mock-claude.sh"
  {
    printf '#!/bin/sh\n'
    printf 'cat > "%s"\n' "$RUN_PROMPT"
    printf 'printf "mock-claude review output\\nVERDICT: pass\\n"\n'
  } >"$_mock"
  chmod +x "$_mock"
