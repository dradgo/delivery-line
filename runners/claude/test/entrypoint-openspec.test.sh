#!/bin/sh
# Story 3a-8 — entrypoint unit tier for the opt-in OpenSpec authoring layer (claude).
#
# Runs the REAL entrypoint.sh with NO container, using the documented
# DELIVERYLINE_*_DIR overrides + a mock Claude CLI (captures its stdin so we can
...
#   * flag ON spec-investigation -> the prompt file is augmented with the fence
...
# Run locally: `sh runners/claude/test/entrypoint-openspec.test.sh`
# (Linux / WSL2 / Git Bash). Mirrors runners/codex/test/entrypoint-openspec.test.sh.
set -eu
...
WORKROOT="$(mktemp -d "${TMPDIR:-/tmp}/claude-ep-spec-authoring.XXXXXX")"
...
run_stage() {
  ...
  # Mock Claude CLI: capture the prompt (stdin) for assertions, emit canned stdout
...
assert_contains "flag-on spec augments the prompt with the fence convention" \
