...
# OPEN CLARIFYING QUESTIONS directive (claude).
#
...
# pins the missing half: the SPEC (investigation) stage prompt must carry the directive that
# tells the model how + when to raise the fenced clarifications block, and NO other stage may.
#
# Runs the REAL entrypoint.sh with NO container, using the documented DELIVERYLINE_*_DIR
# overrides + a mock Claude CLI that captures its stdin (the prompt) so we can assert the
# directive is present on the spec stage and absent elsewhere (stage-gated, mirroring the
# 3d-2 advisory-review directive). Mirrors runners/codex/test/entrypoint-clarifications-directive.test.sh.
#
# POSIX sh, dependency-free besides `node` (for the real runner.mjs prepare/build).
# Run locally: `sh runners/claude/test/entrypoint-clarifications-directive.test.sh`
set -eu
...
# The marker phrase + the exact fence token the directive must teach the model to emit. The
# fence token MUST match lib/runner.mjs splitClarificationsFence's OPEN regex (^```clarifications$).
DIRECTIVE_MARKER="OPEN CLARIFYING QUESTIONS"
FENCE_TOKEN='```clarifications'

NODE_BIN="${NODE_BIN:-node}"
FAILURES=0
WORKROOT="$(mktemp -d "${TMPDIR:-/tmp}/claude-ep-clarif-directive.XXXXXX")"
...
# Runs the entrypoint once for $stage; returns via globals RUN_RC + RUN_PROMPT (the stdin the
# mock CLI received). For the review stage the produced output artifact is referenced under
# /workspace/input; a missing one is irrelevant here — we only inspect the prompt.
run_stage() {
  _stage="$1"
  ...
  _mock="$_w/mock-claude.sh"
  {
    printf '#!/bin/sh\n'
    printf 'cat > "%s"\n' "$RUN_PROMPT"
    printf 'printf "mock-claude stage output\\n"\n'
  } >"$_mock"
  chmod +x "$_mock"
