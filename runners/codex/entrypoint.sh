...
case "$ARTIFACT_TYPE" in
  ...
  review)
    ...
    if [ "${DL_SPLIT_PROPOSAL_REQUESTED:-}" = "true" ]; then
      ...
      # SPECIFICATION itself (not an implementation): there is no patch, no repo checkout, and no
      # approved-spec baseline at this gate. The generic 3d-2 prompt made the model hunt for build
      # output / a git repo and fail (false negative). Review the SPECIFICATION's own quality
      # instead, and end with the same parseable verdict marker.
      PROMPT_INSTRUCTION="OPERATING MODE: ADVISORY SPECIFICATION REVIEW (read-only). You are an independent reviewer in a governed delivery pipeline. A DESIGN SPECIFICATION authored for the ticket below is available under /workspace/input (the reviewed artifact). This is the specification-approval gate: there is NO implementation, patch, code change, or repository checkout to inspect, and the specification is NOT yet approved — do NOT look for or require any of them, and their absence is NEVER a defect. Review the SPECIFICATION ITSELF: does it correctly and completely address the ticket, is the proposed approach sound, are the affected modules/files and acceptance criteria clear and appropriately scoped, and does it leave any listed open clarification questions unresolved. Do NOT modify, create, or delete any files. After your assessment, end your response with EXACTLY one line and nothing after it: 'VERDICT: pass' (no blocking concerns), 'VERDICT: concern' (non-blocking concerns the human should weigh), or 'VERDICT: fail' (a blocking defect in the specification itself)."
    elif [ "${DL_REVIEWED_ARTIFACT_TYPE:-}" = "implementationPlan" ]; then
      # Story 3e-3 follow-up — PLAN-phase advisory review. The reviewed artifact is the
      # IMPLEMENTATION PLAN (a design document), reviewed against the approved specification before
      # human approval. The plan is NOT the implementation: no patch, code change, or repository
      # checkout exists yet, so the generic implementation-review prompt made the model demand a
      # modified pom.xml / repo and fail (false negative). Review the PLAN's fidelity to the approved
      # spec instead, and end with the same parseable verdict marker.
      PROMPT_INSTRUCTION="OPERATING MODE: ADVISORY IMPLEMENTATION-PLAN REVIEW (read-only). You are an independent reviewer in a governed delivery pipeline. An IMPLEMENTATION PLAN authored for the ticket below is available under /workspace/input (the reviewed artifact), to be reviewed against the approved specification. This is the plan-approval gate: NO implementation, patch, code change, or repository checkout exists yet — do NOT look for or require any of them, and their absence is NEVER a defect. Review the PLAN ITSELF: does it fully and faithfully realize the approved specification, are the steps concrete, ordered, and complete, and does it omit or contradict anything the specification requires. Do NOT modify, create, or delete any files. After your assessment, end your response with EXACTLY one line and nothing after it: 'VERDICT: pass' (no blocking concerns), 'VERDICT: concern' (non-blocking concerns the human should weigh), or 'VERDICT: fail' (a blocking defect in the plan itself)."
    else
