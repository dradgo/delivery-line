...
**This is a reservation, not an implementation.** Epic 2 ships no Compare-Mode code: story 2.17's Compare-Mode entry control already ships **disabled** in E2 (`ArtifactReviewPanel.tsx` — `compareEnabled` defaults `false`). This section documents the mobile UX pattern Epic 4 must honor so the responsive contract is settled before that code lands.
