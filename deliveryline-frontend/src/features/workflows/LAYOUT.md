...
| -------------------- | ----------------------------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Left navigation rail | `<nav aria-label="Workflow navigation">`  | `w-64` (16rem) — **fixed**                                              | Tablet: stays inline. Mobile: behind a drawer (`Open workflow navigation` toggle).                                         |
| Central main pane    | `<main id="main-content">`                | `flex-1 min-w-[36rem]` — **shrinks last**                               | Carries the `min-w-[36rem]` floor at every breakpoint. Width never drops below this while the right panel can still yield. |
| Right context panel  | `<aside aria-label="Supporting context">` | `w-80` (20rem) when occupied, `w-12` (3rem) when empty — **shrinkable** | Tablet: drops inline, returns as a right-side `Sheet` drawer behind the `Supporting context` toggle. Mobile: drawer only.  |

### Why these tokens

- The UX spec deliberately states **no pixel widths** for the panes (story spec line 175). The numbers above are chosen to honor: a nav rail wide enough for legible queue/status labels (~16rem); a context panel wide enough to host the Run Context Strip without competing with the artifact (~20rem); and a main pane minimum generous enough for `prose` line length on the artifact (~36rem).
